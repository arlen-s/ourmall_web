	...
	<div class="content">
		...
		<div class="category" v-if="setting.categorys.length > 0">
			<template v-for="(item, index) in setting.categorys">
				<div @click="goMore(item)" v-if="item != '{}'" :key="index" class="items"
					:style="{ width: itemWidth + '%' }">
					<div class="item-inner" :style="{ paddingBottom: proportionHeight + '%' }">
						<img v-if="item.themeUrl" :style="{ 'object-fit': setting.fillWay }" :src="item.themeUrl"
							alt="">
						<img v-if="!item.themeUrl" :style="{ 'object-fit': setting.fillWay }"
							src="../../../../public/images/none-img.png" alt="">
					</div>
...
<script>
	export default {
		...
		created() {
			let sumCnt = this.setting.cntPerRows * this.setting.rows
			this.imgList = [];
			for (var i = 0; i < sumCnt; i++) {
				if (i < 6) {
					this.imgList.push(require(`../../../../src/assets/images/store-design/category${i+1}.png`))
				} else {
					this.imgList.push(require(`../../../../public/images/none-img.png`))
				}
			}
		},
		methods: {
