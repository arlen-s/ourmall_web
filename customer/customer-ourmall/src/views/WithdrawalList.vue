			...
			<el-row :gutter="15" class="mg-b-20">
				<el-col :span="24">
                    ...
                    <el-form-item  :label="$t('提现方式')">
											<el-select filterable remote :placeholder="$t('All')" clearable
										...
										<el-form-item :label="$t('CreateDate')">
											<el-date-picker value-format="yyyy-MM-dd" v-model="filterParams.date"
											 placement="top"
												...
												align="right" unlink-panels :range-separator="$t('to')"
												:start-placeholder="$t('startDate')"
												:end-placeholder="$t('endDate')">
											</el-date-picker>
										</el-form-item>
										<el-form-item :label="$t('accountPayee')">
											<el-select filterable remote :placeholder="$t('All')" clearable
										...
										<el-form-item>
											<el-button type="primary" @click="filterItem">{{$t("Filter")}}
											</el-button>
											<el-button type="danger" @click="clearFilter">{{$t("Clear")}}
											</el-button>
										</el-form-item>
									</el-form>
								</div>
						...
						>
							<el-table-column  :label="$t('customersName')">
								<template slot-scope="scope">
									<span>{{scope.row.customerName || '--'}}</span>
								</template>
							</el-table-column>
							<el-table-column :label="$t('提现方式')">
								<template slot-scope="scope">									
							...
							<el-table-column :label="$t('状态')">
								<template slot-scope="scope">									
									<span v-if="scope.row.drawStatus == 1">{{$t('待审核')}}</span>
                  <span v-else-if="scope.row.drawStatus == 2">{{$t('通过')}}</span>
									<span v-else>{{$t('拒绝')}}</span>
								</template>
							</el-table-column>	              					
							<el-table-column :label="`${$t('tradeAmount')}`">
								<template slot-scope="scope">
