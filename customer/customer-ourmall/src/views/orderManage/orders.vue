<template>
	<div class="contentpanel my-custom-page">
		<div class="pagetitle" ref="pageHeader">
			...
			<div class="right">
				<el-button type="primary" v-if="status == 1 || status == 2" @click="updateFn" :disabled="isDisabled">
					Update order</el-button>
				<el-button type="primary" v-if="status == 1" @click="ImportFn">Import order</el-button>
				<!--  v-if="status == 1 || (status == 2 && activeName == 1)" -->
				<el-button v-if="!(status == 2 && (activeName == 2 || activeName == 3))" type="primary" @click="exportOrder">
			...
			<el-row :gutter="15" class="mg-b-20">
				<el-col :span="24">
					<el-card>
								...
								<el-tabs v-model="activeName" @tab-click="handleClick">
									<div ref="pageFilter" class="page-filter">
										<el-form :inline="true" size="mini">
											<template v-if="status == 2 && (activeName == 2 || activeName == 3)">
												<el-form-item>
													<el-input placeholder="Payment Number"
														v-model="filterParams.sysCode" clearable
														@keyup.enter.native="filterItem"
														@clear="clearFilter('sysCode')"></el-input>
												</el-form-item>
												<el-form-item label="Payment time">
													<el-date-picker value-format="yyyy-MM-dd" key="date2"
														v-model="filterParams.date2" @clear="clearFilter('date2')"
														@change="dateChange2" type="daterange" align="right"
											...
											<template v-else>
												<el-form-item>
													<el-input placeholder="Your No." v-model="filterParams.codeName"
														clearable @keyup.enter.native="filterItem"
														@clear="clearFilter('codeName')"></el-input>
												</el-form-item>
												<el-form-item>
													<el-input placeholder="Order Number" v-model="filterParams.orderId"
														clearable @keyup.enter.native="filterItem"
														@clear="clearFilter('orderId')"></el-input>
												</el-form-item>
												<el-form-item>
													<el-input placeholder="Customer" v-model="filterParams.receiverName"
														clearable @keyup.enter.native="filterItem"
														@clear="clearFilter('receiverName')"></el-input>
												</el-form-item>
												<el-form-item label="Shop name: ">
													<el-select filterable remote placeholder="all" clearable
														v-model="filterParams.accountIds" @change="filterItem"
														@clear="clearFilter('accountIds')">
														<el-option v-for="opt in storeList" :key="opt.id"
															:label="opt.shopName" :value="opt.id"></el-option>
													</el-select>
												</el-form-item>
											...
											<el-form-item class="btns-wrap">
												<template v-if="status == 1 && activeName == 1">
													<el-button type="primary" @click="quoteFn">Create a quote
													</el-button>
													<el-button type="primary" @click="quoteCancelFn">Quit quotation
													</el-button>
												</template>
												<template v-if="status == 2 && activeName == 1">
													<el-button type="primary" @click="payFn">Bulk payment</el-button>
													<el-button type="primary" @click="payCancelFn">Cancel payment
													</el-button>
												</template>
												<template v-if="status == 4 && activeName == 1">
													<el-button type="primary" @click="syncFn">Manually synced
													</el-button>
												</template>
												<template v-if="status == 4 && (activeName == 2 || activeName == 3)">
													<el-button type="primary" @click="BatchContinuePay(1)">Continue Pay
													</el-button>
												</template>
												<template v-if="status == 4 && ( activeName == 3)">
													<el-button type="primary" @click="BatchContinuePay(2)">Re quote
													</el-button>
												</template>
											</el-form-item>
										</el-form>
									...
									<el-tab-pane v-for="(item, index) in tabList" :key="index" :name="item.name+''">
										<span slot="label">
											{{item.title}}
											<span class="tx-danger">({{customerAllOrderCntText(item.id)}})</span>
										</span>
									</el-tab-pane>
								</el-tabs>
						...
						<!-- table -->
						<template v-if="(status == 1 || status == 2 || status == 4) && activeName == 1">
							<el-table key="gridTable" stripe style="width: 100%" ref="gridTable" :data="items"
								...
								<el-table-column label="Your shop order num" width="100">
									<template slot-scope="scope">
										<el-popover width="1000" trigger="hover" placement="top-start"
											:ref="'popover' + scope.row.codeName">
											<div class="items">
												<div v-for="(item,i) in scope.row.items"
													:key="scope.row.orderId + scope.row.codeName + i"
													class="d-flex mg-b-15">
													<el-checkbox v-model="item.check"></el-checkbox>
													<img :src="item.imgUrl" width="80" height="80"  class="mg-l-10"/>
													<div class="mg-l-10">
														<p style="width: 800px;" class="mg-b-10">
															<span v-if="!item.vendorProductId">{{item.name}}</span>
															<el-link v-else type="primary" :href="`/item/${item.vendorProductId}/${item.vendorProductName}.html`" target="_blank">
														...
														<p class="mg-b-10">
															<template v-if="item.isManage == 2">
																<span
																	style="width: 450px;display: inline-block;color: #F56C6C;">SKU:
																	{{item.vendorSku + 'Unable to purchase'}}</span>
															</template>
															<template v-else>
																<template v-if="item.status != 1">
																	<span
																		style="width: 450px;display: inline-block;color: #F56C6C;">SKU:
																		{{item.vendorSku ? `${item.vendorSku} Products Off shelf` : 'null     Please make an offer'}}</span>
																</template>
																<template v-else>
																	<span
																		style="width: 450px;display: inline-block;">SKU:
																		{{item.vendorSku || 'null     Please make an offer'}}</span>
																</template>
															</template>
															<span style="width: 200px;display: inline-block;">
																Weight：{{item.vendorProductWeight || '---'}} g
															</span>
														</p>
														<p>
															<span style="width: 200px;display: inline-block;">
															...
															<span style="width: 250px;display: inline-block;">
																...
																<span v-if="item.vendorSku">
																	<template v-if="(scope.row.discountType == 1 || scope.row.discountType == 2) && item.isManage != 2">
																		<span>$ {{Number((item.amount - item.amount / scope.row.productAmount * scope.row.couponAmount) / item.quantity).toFixed(2)}}&nbsp;</span>
																		<span style="color: red; text-decoration:line-through;">
																			<span style="color: #606266;">$ {{Number(item.vendorPrice).toFixed(2)}}</span>
																		</span>
																	</template>
																	<template v-else>$
																		{{Number(item.vendorPrice).toFixed(2)}}</template>
																</span>
																<span style="color: #F56C6C;" v-else>Please make an
																	offer</span>
															</span>
															<span
															...
															<span style="width: 100px;display: inline-block;text-align: right;">
																<template v-if="(scope.row.discountType == 1 || scope.row.discountType == 2) && item.isManage != 2">
																	...
																	<span style="color: red; text-decoration:line-through;">
																		<span style="color: #606266;">$ {{Number(item.amount).toFixed(2)}}</span>
																	</span>
																</template>
																<template v-else>$
													...
													<!-- 删除 -->
													<div class="d-flex" style="flex: 1;justify-content: flex-end;margin-right:5px" v-if="item.isManage != 2">
														<el-link type="primary" @click="deleteOrder(item,scope.row)">Delete</el-link>
													</div>
												</div>
											</div>
									...
									<template slot-scope="scope">
										<el-popover width="1000" trigger="hover" placement="top-start"
											...
											<div slot="reference" class="name-wrapper">
												...
												<!-- <span style="display: inline-block;height: 26px;width: 26px;line-height: 26px;text-align: center;background: #5c6ac4;color: #fff;margin-left: 10px;font-size: 10px!important;" v-if="status != 1 && scope.row.isSplit == 1">
													Split
												</span> -->
												<img v-if="status != 1 && scope.row.isSplit == 1" style="margin-left: 10px;" src="../../../public/images/split2.png" />
											</div>
										</el-popover>
									</template>
								</el-table-column>
								...
								<el-table-column label="Created Time" width="200">
									<template slot-scope="scope">
										<span>{{moment(scope.row.timeCreated).format("YYYY-MM-DD HH:mm:ss") || '---'}}</span>
									</template>
								</el-table-column>
									...
									<el-table-column label="Shipping Method" width="150">
										<template slot-scope="scope">
											<template
												...
												<el-select v-model="scope.row.shippingMethodId"
													@change="filterShippingMethod($event,scope.row)" size="small">
													<el-option v-for="shippingMethod in scope.row.shippingMethodArr"
														:key="shippingMethod.id" :label="`${shippingMethod.nameDisplay ? shippingMethod.nameDisplay : shippingMethod.name} [${shippingMethod.agingDaysBegin?shippingMethod.agingDaysBegin:0}-${shippingMethod.agingDaysEnd?shippingMethod.agingDaysEnd:0}days]`"
														:value="shippingMethod.id"></el-option>
												</el-select>
											</template>
											<div class="tx-middle" v-else>--- <i @click="shippedFn(scope.row)" class="el-icon-warning" style="font-size: 20px;cursor:pointer;"></i></div>
										</template>
									</el-table-column>
									<el-table-column :label="`Shipping cost(${$showSybmol()})`" width="150">
										<template slot-scope="scope">
											<span v-if="scope.row.discountType == 3">
												<span>0&nbsp;</span>
												<span style="color: red; text-decoration:line-through;">
													<span style="color: #606266;">{{scope.row.shippingMethodItem ? $exchangeRate(scope.row.shippingMethodItem.fee) : '---'}}</span>
												</span>
											</span>
											<span v-else>{{scope.row.shippingMethodItem ? $exchangeRate(scope.row.shippingMethodItem.fee) : '---'}}</span>
										</template>
									</el-table-column>
								</template>
								<template v-if="status == 2">
									<el-table-column :label="`Total(${$showSybmol()})`" width="150">
										<template slot-scope="scope">
											<span>{{scope.row.totalAllAmount ? $exchangeRate(scope.row.totalAllAmount) : '---'}}</span>
										</template>
									</el-table-column>
								</template>
								<template v-if="status == 4">
									<el-table-column :label="`pay total(${$showSybmol()})`" width="100">
										<template slot-scope="scope">
											<span v-if="scope.row.discountType">
												<span>{{scope.row.payAmount ?  $exchangeRate(scope.row.payAmount) : '---'}}&nbsp;</span>
												<span style="color: red; text-decoration:line-through;">
													<span style="color: #606266;">{{scope.row.realAmount ? $exchangeRate(scope.row.realAmount) : '---'}}</span>
												</span>
											</span>
											<span v-else>{{scope.row.realAmount ? $exchangeRate(scope.row.realAmount) : '---'}}</span>
										</template>
									</el-table-column>
								</template>
								<el-table-column label="Action" width="120" fixed="right">
									<template slot-scope="scope">
										<div>
											<el-link v-if="status == 1" class="mg-r-20" type="primary"
												@click="quoteFn('row',scope.row)">
												Quote
											</el-link>
											<el-link
												v-if="status == 2 && scope.row.shippingMethodArr && scope.row.shippingMethodArr.length"
												class="mg-r-20" type="primary" @click="payFn('row',scope.row)">
												Pay
											</el-link>
											<el-link v-if="status == 4" class="mg-r-20" type="primary"
								...
								<el-table-column label="Remark" width="100">
									<template slot-scope="scope">
										<div><div>{{ scope.row.remark || '---'}}</div></div>
									</template>
								</el-table-column>
								</template>
							</el-table>
						...
						<template v-else-if="status == 2">
							<el-table key="gridTable2" stripe style="width: 100%" ref="gridTable" :data="items"
								tooltip-effect="dark" @selection-change="handleSelectionChange"
								:row-key="(row) => row.id">
								<el-table-column type="selection" width="55" :reserve-selection="true">
								</el-table-column>
								...
								<el-table-column :label="`Payment amount(${$showSybmol()})`" width="200">
									<template slot-scope="scope">
										<span v-if="scope.row.discountType">
											<span>{{scope.row.payAmount ? $exchangeRate(scope.row.payAmount) : '---'}}&nbsp;</span>
											<span style="color: red; text-decoration:line-through;">
												<span style="color: #606266;">{{scope.row.totalAmount ? $exchangeRate(scope.row.totalAmount) : '---'}}</span>
											</span>
										</span>
										<span v-else>{{scope.row.realAmount ? $exchangeRate(scope.row.realAmount) : '---'}}</span>
									</template>
								...
								<template v-if="activeName == 2">
									<el-table-column label="Inventory lock remaining time" width="200">
										<template slot-scope="scope">
								...
								<el-table-column label="Order quantity" min-width="100">
									<template slot-scope="scope">
										<el-link v-if="scope.row.shopifyOrderCnt" type="primary"
								...
								<template v-if="activeName == 2">
									<el-table-column label="Status" width="200">
										<template slot-scope="scope">
											<template>
												<span
													v-if="scope.row.accountPayment">{{statusPayArr[scope.row.accountPayment.status] ? statusPayArr[scope.row.accountPayment.status].text : '---'}}</span>
												<span v-else>---</span>
											</template>
										</template>
									</el-table-column>
								...
								<template v-if="activeName == 3">
									<el-table-column label="Status" width="200">
										<template slot-scope="scope">
											<template>
												<span
													v-if="scope.row.paymentType">{{paymentTypeArr[scope.row.paymentType] ? paymentTypeArr[scope.row.paymentType].text : '---'}}</span>
												<span v-else>---</span>
											</template>
										</template>
									</el-table-column>
								</template>
								<el-table-column v-if="activeName == 2" label="Action" width="120" fixed="right">
									<template slot-scope="scope">
										<div>
											<el-link class="mg-r-20" type="primary"
												@click="payCancelFn('row',scope.row)">
												Cancel payment
											</el-link>
											<el-link class="mg-r-20" type="primary" @click="repayFn(scope.row)">
												Repay
											</el-link>
										</div>
									</template>
						...
						<template v-else>
							<el-table key="gridTable3" stripe style="width: 100%" ref="gridTable" :data="items"
								tooltip-effect="dark" @selection-change="handleSelectionChange"
								:row-key="(row) => row.id">
								<el-table-column type="selection" width="55" :reserve-selection="true">
								</el-table-column>
												...
												<div v-for="(item,i) in scope.row.items"
													...
													<div class="mg-l-10">
														...
														<p class="mg-b-10">
															<template v-if="item.isManage == 2">
																<span
																	style="width: 350px;display: inline-block;color: #F56C6C;">SKU:
																	{{item.vendorSku + 'Unable to purchase'}}</span>
															</template>
															<template v-else>
																<template v-if="item.status != 1">
																...
																<template v-else>
																	<span
																		style="width: 350px;display: inline-block;">SKU:
																		{{item.vendorSku || 'null     Please make an offer'}}</span>
																</template>
															</template>
														</p>
								...
								<!-- 1.6.0隐藏 -->
								<el-table-column v-if="status == 4 && (activeName == 2 || activeName == 3)"
									label="Operate" width="150">
									<template slot-scope="scope">
										<div>
											<el-link type="primary" v-if="activeName == 3"
												@click="AbnormalContinuePay(scope.row, 2)">Re quote</el-link>
										</div>
										<div>
											<el-link class="" type="primary" @click="AbnormalContinuePay(scope.row, 1)">
								...
								<template v-if="status == 3 || status == 5 || status == 6">
									<el-table-column label="Shipping Method" width="250">
										<template slot-scope="scope">
											<span>{{scope.row.trackCompany ? `${scope.row.trackCompany}${scope.row.trackNumber 
											? `:${scope.row.trackNumber} [${scope.row.agingDaysBegin?scope.row.agingDaysBegin:0}-${scope.row.agingDaysEnd?scope.row.agingDaysEnd:0}days]` 
											: `[${scope.row.agingDaysBegin?scope.row.agingDaysBegin:0}-${scope.row.agingDaysEnd?scope.row.agingDaysEnd:0}days]`}` : 'No logistics'}}</span>
										</template>
									</el-table-column>
									<el-table-column :label="`Shipping cost(${$showSybmol()})`" width="150">
										<template slot-scope="scope">
											<span v-if="scope.row.discountType == 3">
												<span>0&nbsp;</span>
												<span style="color: red; text-decoration:line-through;">
													<span style="color: #606266;">{{scope.row.trackFee ? $exchangeRate(scope.row.trackFee) : '---'}}</span>
												</span>
											</span>
											<span v-else>{{scope.row.trackFee ? $exchangeRate(scope.row.trackFee) : '---'}}</span>
										</template>
									</el-table-column>
									<el-table-column :label="`pay total(${$showSybmol()})`" width="150">
										<template slot-scope="scope">
											<span v-if="scope.row.discountType">
												<span>{{scope.row.payAmount ? $exchangeRate(scope.row.payAmount) : '---'}}&nbsp;</span>
												<span style="color: red; text-decoration:line-through;">
		...
		<el-dialog :visible="dialogPay.isShow" :loading="dialogPay.loading" class="pay-dialog" width="30%"
			...
			<div class="pay-info">
				...
				<div v-if="dialogPay.amount">
					Total Amount:
					<b class="tx-danger">{{ $exchangeRate(dialogPay.amount.toFixed(2)) }} {{$showSybmol()}}</b>
				</div>
			</div>
			<template v-if="dialogPay.step == 1">
				<div class="pay-platform">
					...
					<div class="platform-select">
						...
						<div v-if="(invoiceInfo && invoiceInfo.relationship) && invoiceInfo.relationship.bonusStatus == 1"
							...
							<a href="javascript:;" :class="{ active: dialogPay.platformType == 6 }"
								...
								<div class="d-flex">
									...
									<span>Bonus(${{ Number((invoiceInfo.relationship || {}).bonus) }})</span>
								</div>
								<div class="active" v-show="dialogPay.platformType != 6">
									<img v-if="invoiceInfo && invoiceInfo.relationship && invoiceInfo.relationship.bonus > 0"
										src="../../assets/pay/Basic.png" alt="" height="20">
						...
						<div
							v-if="dialogPay.payTypes.length == 0 && ((invoiceInfo && invoiceInfo.relationship) && invoiceInfo.relationship.bonusStatus == 2)">
		...
		<el-dialog :visible="abnormalStatus" :before-close="abnormalClose" width="550px">
			<div class="abnormal-dialog">
				<h1 v-if="abnormalType == 1 && !canPay">The selected orders are abnormal and payment cannot be continued
				</h1>
				<h1 v-if="abnormalType == 1 && canPay">Payment can continue after excluding the following goods</h1>
				<h1 v-if="abnormalType == 2">After removing the off shelf goods, the order will be returned to the price
					to be quoted</h1>
				<el-table :data="abnormalData">
					...
					<el-table-column label="Status" prop="reason">
					</el-table-column>
				</el-table>
				<div class="btn-area">
					<el-button size="small" v-if="canPay" @click="abnormalClose">Cancel</el-button>
					<el-button size="small" :loading="abnormalBtnVisible" type="primary" @click="AbnormalReClick">
						Confirm</el-button>
				</div>
			</div>
		</el-dialog>
		<el-dialog title="Create Order" :visible="createVisible" :before-close='ImportCancel' width="400px">
			<div class="create-order">
				<el-upload class="upload-demo" ref="upload"
					accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" :auto-upload="false"
					:file-list="uploadfileList" :on-change="fileBeforeUpload" action="">
					<el-button slot="trigger" size="small" type="primary">选取文件</el-button>
					<div class="el-upload__tip" slot="tip">
						<div>点击下载 <a class="template" @click="downloadFile">订单导入模板</a></div>
...
<script>
	...
	export default {
		data() {
			return {
				...
				rowsPerPage: localStorage.getItem("c_ordersRowsPerPage") ?
					Number(localStorage.getItem("c_ordersRowsPerPage")) : 10,
				items: [],
				total: 0,
				totalPage: 0,
		...
		mounted() {
			...
			let status = this.$route.params.status ? Number(this.$route.params.status) : 1;
			let paystatus = this.$route.query.paystatus ? Number(this.$route.query.paystatus) : 1;
			if (status == 2 && this.activeName == "1" && (paystatus == 2 || paystatus == 3)) {
		...
		methods: {
			...
			deleteOrder(i,data){
               ...
               this.$confirm('确定删除该商品？', 'Tips', {
					...
					this.$apiCall('api.ShopifyOrder.delshopifyItem',{
						ids:ids
					},r=>{ 
						if(r.ErrorCode == 9999){
							this.getItem()
			...
			AbnormalReClick() {
				...
				if (this.batchChange) {
					// 批量
					this.$apiCall('api.ShopifyOrder.reOfferOrPay', {
			...
			BatchContinuePay(type) {
				...
				this.$apiCall('api.ShopifyOrder.checkOfferOrPay', {
					ids: this.abnormalSelection,
					type: type
				}, (r) => {
					if (r.ErrorCode == 9999) {
						...
						} else if (this.abnormalData.length == 0 && this.abnormalType == 2) {
							this.$elementMessage('No goods need to be eliminated', 'error')
						} else if (this.abnormalData.length == 0 && this.abnormalType == 1) {
			...
			shopifyOrderCnt(item) {
				this.$showLoading();
				this.$apiCall("api.ShopifyOrder.shopifyOrders", {
					invoiceId: item.id,
					isAll: true
				}, (r) => {
					if (r.ErrorCode == "9999") {
			...
			filterShippingMethod(e, item) {
				...
				if (item.shippingMethodItem) { //有运费加上运费
					this.$set(item, "totalAllAmount", (item.itemAmount ? Number(item.itemAmount) : 0) + Number(item
						.shippingMethodItem.fee));
				} else {
					this.$set(item, "totalAllAmount", item.itemAmount ? Number(item.itemAmount) : 0);
				}
			...
			countDown(item) {
				...
				item.clock = setInterval(() => {
					...
					let end = moment.unix(time).add(2, 'days').format('YYYY-MM-DD HH:mm:ss'); //结束时间
					let today = moment().format('YYYY-MM-DD HH:mm:ss'); //今天时间
					let diff = moment(end).diff(moment(today)); //时间差
					if (diff > 0) {
						if (moment.duration(diff).days() > 0) {
							timeLeft =
								`${moment.duration(diff).hours() + 24} : ${moment.duration(diff).minutes()} : ${moment.duration(diff).seconds()}` //格式化为需要的格式 这里是时分秒	
						} else {
							timeLeft =
								`${moment.duration(diff).hours()} : ${moment.duration(diff).minutes()} : ${moment.duration(diff).seconds()}` //格式化为需要的格式 这里是时分秒	
						}
					} else {
						timeLeft = "null";
			...
			orderPay() {
				...
				let type = "";
				let paymentId = "";
				if (this.dialogPay.platformType && !Number(this.dialogPay.platformType) && this.dialogPay.platformType
					.indexOf('underline') != -1) {
			...
			openPay(obj, type) {
				...
				this.$apiCall(url, params, (r) => {
					...
					if (r.ErrorCode == "9999") {
						...
						localStorage.setItem('accountPayment', r.Data.Results.accountPayment ? JSON.stringify(r
							.Data.Results.accountPayment) : "");
						if (r.Data.Results.shopifyOrders) {
							for (let index in r.Data.Results.shopifyOrders) {
								items.items.push(Object.assign({}, r.Data.Results.shopifyOrders[index], {
									orderId: index
								}))
							}
				...
				this.$apiCall("api.Invoice.getBonus", {}, (r) => {
					if (r.ErrorCode == "9999") {
						...
						this.invoiceInfo.relationship.bonusStatus = r.Data.Results.bonusStatus;
					} else {
						this.$elementMessage(r.Message, "error");
					}
				})
				this.$apiCall("api.AccountPayment.getPaymentAccounts", {}, (r) => {
					if (r.ErrorCode == "9999") {
						...
						r.Data.Results.forEach(item => {
							item.icon = require('@/assets/pay/n' + Number(item.accountType) + '.png');
			...
			sync(type, item) {
				...
				this.$showLoading();
				this.$apiCall("api.ShopifyOrder.setNotAbnormal", {
					ids: ids
				}, (r) => {
					if (r.ErrorCode == "9999") {
						this.$elementMessage("Successfully", "success");
			...
			continuePay() {
				this.openPay(this.dialogCheckStock.params, this.dialogCheckStock.openType);
			},
			payFn(type, item) {
				if (type == 'row') {
			...
			checkOpenPay(item, type) {
				...
				let obj = {};
				if (item) {
					obj[item.id] = item.shippingMethodItem ? item.shippingMethodItem.id : "";
				} else {
				...
				this.$apiCall("api.ShopifyOrder.checkSkuStock", params, (r) => {
					...
					if (r.ErrorCode == "9999") {
						...
						if (!r.Data.Results || !r.Data.Results.abnormalSkus || !r.Data.Results.abnormalSkus
							.length) {
							this.openPay(obj, type);
						} else {
							//打开无库存弹层
			...
			payCancel(type, text, item) {
				this.$confirm(text, "", {
					...
					if (type == 'row') {
						//取消支付
						this.$apiCall("api.ShopifyOrder.cancelPay", {
							id: item.id
						}, (r) => {
							if (r.ErrorCode == "9999") {
								this.$elementMessage("Successfully", "success");
			...
			quote(type, item) {
				this.$confirm("Are you sure to initiate a quotation for the selected order?", "", {
					confirmButtonText: "Confirm",
					cancelButtonText: "Cancel",
					type: ''
				}).then(() => {
					...
					this.$showLoading();
					this.$apiCall("api.ShopifyOrder.commitOffer", {
						ids: ids
					}, (r) => {
						if (r.ErrorCode == "9999") {
							this.$elementMessage("Successfully", "success");
			...
			dateChange2() {
				if (this.filterParams.date2 && this.filterParams.date2.length > 1) {
					this.filterParams.dateFrom2 = moment(
						this.filterParams.date2[0]
					).format("YYYY-MM-DD");
					this.filterParams.dateTo2 = moment(
			...
			customerAllOrderCntText(id) {
				let text = 0;
				if (id == 22) {
					text = this.customerAllOrderCnt.waitCustomerPayCnt ? this.customerAllOrderCnt.waitCustomerPayCnt : 0;
				} else if (id == 23) {
					text = this.customerAllOrderCnt.waitVendorConfirmCnt ? this.customerAllOrderCnt.waitVendorConfirmCnt :
						0;
				} else {
					...
					if (r.ErrorCode == 9999) {
						...
						this.items.forEach(item => {
							...
							if (item.shippingMethodArr && item.shippingMethodArr.length) {
								...
								this.$set(item, "shippingMethodFee", Number(item.shippingMethodArr[0]
								.fee));
							}
							this.$set(item, "shippingJsonInfo", item.shippingJson ? JSON.parse(item
								.shippingJson) : "");
							if (item.items.length) {
								item.items.forEach(obj => {
									this.$set(obj, "totalPrice", Number(obj.vendorPrice) * Number(
										obj.quantity));
								})
								// if(item.shippingMethodItem){//有运费加上运费
								// 	this.$set(item,"totalAllAmount",this.countTotal(item.items,'totalPrice') + Number(item.shippingMethodItem.fee));
							...
							// }
							if (item.shippingMethodItem) { //有运费加上运费
								this.$set(item, "totalAllAmount", (item.itemAmount ? Number(item
									.itemAmount) : 0) + Number(item.shippingMethodItem.fee));
							} else {
								this.$set(item, "totalAllAmount", item.itemAmount ? Number(item
									.itemAmount) : 0);
							}
