...
<!--                      {{scope.row.itemSyncFailCnt}} sync failed-->
                         <el-popover
                           placement="bottom"
                           :title="$t('Download tracking information')"
                           width="700"
                           trigger="hover"
                         >
    ...
    <el-dialog :visible.sync="selectedSaveDialog.isShow" :width="'620px'" :close-on-click-modal="false" class="selected-save-dialog">
      <div style="padding: 0 20px 20px;text-align: center;">
        ...
        <el-table :data="selectedSaveDialog.info" border style="width: 100%">
          <el-table-column :label="`#${$t('Invoice Number')}`" prop="code">
            <template slot-scope="scope">
              <span>#{{scope.row.code}}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('Vendor')" prop="vendorName">
            <template >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button :loading="cancelOrderDialog.loading" type="primary" @click="postCanceOrder">{{$t('Confirm')}}</el-button>
