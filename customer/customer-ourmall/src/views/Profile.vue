  ...
  <div class="contentpanel profile-page">
    <div class="pagetitle">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        >
        <el-breadcrumb-item>{{$t('My Profile')}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
          ...
          <el-col :span="24">
            <el-card class="mg-b-25">
              <div slot="header" class="clearfix">
                <span class="header-title-text"> {{$t('GENERAL INFORMATION')}} </span>
              </div>
              <el-row :gutter="20">
                <el-col :span="10">
            ...
            <el-card class="mg-b-25">
              <div slot="header" class="clearfix">
                <span class="header-title-text"> {{$t('CONTACT')}} </span>
              </div>
              <el-row :gutter="20">
                <el-col :span="12">
                ...
                <el-col :span="12">
                  <el-form-item label="Phone">
                   ...
                   <el-link type="primary" class="mg-l-10" style="margin-bottom:3px"
                    ...
                    <span v-if="form.phone">{{$t('Change')}}</span>
            ...
            <el-card class="mg-b-25">
              <div slot="header" class="clearfix">
                <span class="header-title-text"> {{$t('SOCIAL')}} </span>
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="12">
                  <el-form-item label="WangWang">
                    ...
                    <el-input
                      v-model="form.wangwang"
                      placeholder="WangWang"
                    ></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
            ...
            <el-card class="mg-b-25">
              <div slot="header" class="clearfix">
                <span class="header-title-text"> {{$t('Notification Settings')}} </span>
              </div>
              <div class="el-row" style="margin-left: -10px; margin-right: -10px;">
              <div class="el-col el-col-12 d-flex" 
              style="margin:0 145px;flex-direction: column;align-items: flex-start;">
                <div class="d-flex" style="margin-bottom:36px">
                  <div style="color: #606266;">{{$t('I want to receive notfications by')}}:</div>
                   <div style="margin:5px 30px 0">
                    <el-checkbox disabled v-model="checked1">Email</el-checkbox>
                    <el-checkbox v-model="checkedPhone">{{$t('Mobile phone')}}</el-checkbox>
                 </div>
               </div>
                  <div style="margin-bottom:36px">
                      <p><el-checkbox disabled v-model="checked1">{{$t('Allow system notifhcations')}}</el-checkbox></p>
                      <p><el-checkbox v-model="checked2">{{$t('Allow Customer Services from ')}} {{$root.$children[0].pName.a}} staffs</el-checkbox></p>
                      <p><el-checkbox v-model="checked3">{{$t('Allow Dropshipping News')}}</el-checkbox></p>
                  </div>
              </div>
              </div>
            </el-card>
    ...
    >
      <el-row :gutter="30">
        <el-col :span="24">
          <el-form>
            <el-form-item
              :label="isPhone==false? 'Email':'Phone'"
              label-width="80px"
            >
            ...
            <el-form-item
              ...
              label-width="80px"
            >
            <vue-simple-verify ref="verify" @success="successFunction" :width="480"
            ...
            {{$t('try again in')}}try again in {{time}} {{$t('seconds')}}
          </el-link>
              </div>
            </el-form-item>
          </el-form>
      ...
      >
        <el-button
        :disabled="!authcode"
          type="primary" @click="saveChange"
        >{{$t('Save')}}</el-button>
        <el-button @click="closeChange">{{$t('Discard')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
    ...
    let t = setInterval(() => {
      if (this.$store.state.userInfo) {
        ...
        if(!this.form.whatsapp && !this.form.skype){
          this.tip =this.$t("Complete your Skype or WhatsApp details to get rewards")
        }else{
          this.tip = this.$t("You have completed the Skype or WhatsApp, you can receive a free gift package.")
        }
  ...
  methods: {
     ...
     this.changeUserEmail();
      ...
      // 获取验证码
        ...
        if(this.isPhone == false){
          ...
          this.$hideLoading();
	        if (r.ErrorCode == '9999') {
	        	this.$elementMessage(this.$t('The captcha was sent successfully'), "success");
	        	vm.time = 60;
    ...
    isCEmail(email) {
     ...
     successFunction(){
          ...
          this.$refs.verify.reset();
          this.econfirmSuccess = false;
          this.failTx = this.isPhone==false ? this.$t('Please fill in the correct Email address'): this.$t('Please fill in the correct Mobile phone');
      	}
    } ,
    save() {
      if (!this.form.name) {
