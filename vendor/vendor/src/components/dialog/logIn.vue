<template>
	<div>
		<el-dialog :visible.sync="show" :width="showType<3 ? '500px' : '700px'" :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false" class="login-dialog"
      style="z-index: 99999"
    >
			<el-card v-if="showType == 1" class="box-card signup-box">
				<div slot="header" class=" tx-bold" style="line-height: 26px;font-size: 18px;padding-left:10px;">
					“{{rid.customerName}}”{{$t('invoice.invitesview')}}
				</div>
				<div style="padding: 0 20px;font-size: 14px;font-weight: 700">
					{{$t('invoice.Signinview')}}
				</div>
				<el-form ref="form" :model="form" :rules="rules" :label-width="labelWidth" :validate-on-rule-change="false">
					<el-form-item prop="userName">
						<el-input autoComplete="off" v-model="form.userName" :disabled="loading" clearable :placeholder="$t('signin.mobile')" @keyup.enter.native="focusPassword"></el-input>
					</el-form-item>
					<el-form-item prop="password">
						<el-input ref="iptPassword" v-model="form.password" :disabled="loading" show-password auto-complete="new-password" :placeholder="$t('signin.pass')" @keyup.enter.native="onSumit('form')"></el-input>
					</el-form-item>
					<div class="d-flex align-items-center justify-content-between">
						<el-form-item style="margin-bottom: 0">
							<el-button type="primary" :loading="loading" @click="onSumit('form')" style=" margin-right: 15px">{{$t('signin.sign')}}</el-button>
						</el-form-item>
					</div>
				</el-form>
			</el-card>
			<el-card v-if="showType == 2" class="box-card signup-box">
				<div slot="header" class=" tx-bold" style="font-size: 18px;padding-left:10px;">
					“{{rid.customerName}}”{{$t('invoice.invitesview')}}
				</div>
				<!-- 注册 -->
				<div style="padding: 0 20px;font-size: 14px;font-weight: 700">
					{{$t('invoice.Signupview')}}
				</div>
				<el-form ref="form" :model="formReg" :rules="rulesReg" :label-width="labelWidth">
					<el-form-item prop="userName">
						<el-input v-model="formReg.userName" :disabled="loading" clearable :placeholder="$t('signin.mobile')"></el-input>
					</el-form-item>
					<el-form-item prop="password">
						<el-input auto-complete="new-password" v-model="formReg.password" :disabled="loading" show-password :placeholder="$t('signin.pass')"></el-input>
					</el-form-item>
					<div style="margin-bottom: 20px">
						<template v-if="!showReSend">
							<div v-if="!mobileSeconds" class="drag dragmobile" :style="{width: (maxwidth + 40) + 'px'}">
								<div class="drag_bg drag_bgmobile"></div>
								<div class="drag_text">{{confirmWords}}</div>
								<div @touchstart="mousedownFn($event)" @mousedown="mousedownFn($event)" class="handler handlermobile handler_bg"></div>
							</div>
							<span v-else>
                <span class=" mg-r-5 tx-success">{{$t('signup.captchaSent')}}</span>{{mobileSeconds}}{{$t('signup.captchaSentSec')}}
							</span>
						</template>
						<span v-else>
              <el-link
                type="primary"
                @click="sendToken2"
              >{{$t('signup.captchaAgain')}}</el-link>
			            </span>
					</div>
					<el-form-item prop="token">
						<el-input v-model="formReg.token" :disabled="loading" :placeholder="$t('signup.captcha')"></el-input>
					</el-form-item>
					<div v-if="error" class="error" style="color:#f56c6c;">
						{{error}}
					</div>
					<div v-if="success" class="success">{{success}}</div>
					<div class="d-flex align-items-center justify-content-between">
						<p class=" d-flex align-items-center">
							<el-checkbox v-model="isAgree" style="margin: 0;">{{$t('signup.agree')}} </el-checkbox>
							<el-link class=" mg-l-5" type="primary" @click="openUserAgreement">{{$t('signup.agreement')}}</el-link>
						</p>
					</div>
					<div class="d-flex"
                    style="flex-direction: column; align-items: end; margin-top: 10px"
                        >	
						<el-form-item style="margin-bottom: 0">
							<el-button type="primary" :loading="loading" style=" margin-right: 15px" @click="onSumit2('form')">{{$t('signup.complete')}}</el-button>
						</el-form-item>

					</div>
					<div class=" text-right">
						<el-link class=" d-inline" @click="toSignin()" style="position: relative; top: 12px;">{{$t('signup.signin')}}</el-link>
					</div>
				</el-form>
			</el-card>
			<el-dialog append-to-body :title="$t('signup.用户注册协议')" :visible.sync="dialogUserAgreementVisible" class="dialog-user-agreement">
      ...
      />Time limit to respond<br/>We try to respond to all
        ...
        <el-button type="primary" @click="dialogUserAgreementVisible = false">{{$t("signup.我知道了")}}</el-button>
      </span>
			</el-dialog>
			<el-card v-if="showType == 3">
				<div slot="header" class=" tx-bold" style="font-size: 18px;">
					{{$t('invoice.viewingvender',{name:rid.customerName,name2:rid.vendorName})}}
					<!--您正在查看客户“{{rid.customerName}}”发送给供应商“{{rid.vendorName}}”的采购订单明细-->
				</div>
				<div style="padding: 10px;">
					<div style="margin-bottom: 30px;">
						{{$t('invoice.inconsistentassociate',{name:userInfo.name,name2:rid.vendorName})}}
						<!--似乎您当前的登录信息“{{userInfo.name}}”与采购单的供应商信息不一致，是否要把当前采购订单的供应商“{{rid.vendorName}}”与您的账号关联在一起？-->
					</div>
					<div class="tx-right">
						<el-button type="primary" :loading="loading" @click="relate">{{$t('invoice.confirm')}}</el-button>
						<el-button type="danger" @click="closeWin">{{$t('invoice.notOrder')}}</el-button>
					</div>
				</div>
			</el-card>
			<el-card v-if="showType == 4">
				<div slot="header" class=" tx-bold" style="font-size: 18px;">
					{{$t('invoice.viewingvender',{name:rid.customerName,name2:rid.vendorName})}}
					<!--您正在查看客户“{{rid.customerName}}”发送给供应商“{{rid.vendorName}}”的采购订单明细-->
				</div>
				<div style="padding: 10px;">
					<div style="margin-bottom: 30px;">
						{{$t('invoice.inconsistentexit',{name:userInfo.name,name2:rid.vendorName})}}
						<!--似乎您当前的登录信息“{{userInfo.name}}”与采购单的供应商信息不一致，是否要退出当前账号并且重新登录“{{rid.vendorName}}”的供应商账号？-->
					</div>
					<div class="tx-right">
						<el-button type="primary" @click="loginAgain">{{$t('invoice.Resign')}}</el-button>
						<el-button type="danger" @click="closeWin">{{$t('invoice.notOrder')}}</el-button>
					</div>
				</div>
			</el-card>
...
<script>
          ...
          userName: [{
            ...
            token: [{
              ...
              trigger: "blur"
				...
				maxwidth: 398 /*拖动最大宽度，依据滑块宽度算出来的*/ ,
      ...
      lang() {
		...
		methods: {
			relate() {
				this.loading = true;
				this.$apiCall(
					"api.Relationship.relationVendorId", {
						relationshipId: this.rid.relationshipId
					},
					r => {
						this.loading = false;
						if(r.ErrorCode == 9999) {
							this.$message({
								message: this.$t('signup.Association'),
								type: "success"
							});
    ...
    isCMobile(mobile) {
      return mobile ? /^1[3-9]\d{9}$/.test(mobile) : false;
    },
    sendToken2() {
    ...
    sendToken() {
      ...
      this.$apiCall(
        "api.User.sendToken",
        { mobile: this.formReg.userName, lang: this.lang },
        (r) => {
          this.loading = false;
          if (r.ErrorCode == 9999) {
...
//              this.success = '注册成功请登录';
                this.successT = setInterval(() => {
                  this.success = "";
                }, 5000);
				this.$message({
					message: this.$t('invoice.success'),
					type: "success"
				});
...
<style>
  .login-dialog{
	...
	.login-dialog .el-dialog__body .el-card__body {
