    ...
    <div class="invoice" v-cloak>
      <div v-if="!fromPage" class="invoice-top" style="height: 60px;">
        <div v-if="loginType ==1 || loginType ==2">
          ...
          >
            <div style="width: 1100px;margin: 0 auto;padding: 0px 30px 0 0;">
              <div v-if="loginType ==2" class="tx-left" style="background-color: rgb(253, 246, 236);border-radius: 4px;border-left: 5px solid rgb(230, 162, 60);margin: 0 0 0 0px;height: 60px; padding: 0 20px;box-shadow: rgba(33, 33, 33, 0.4) 0px 1px 4px 0px;">
                <p style="color: #606266;font-weight: 600;margin-bottom: 0;font-size: 14px;line-height: 60px;" class="d-flex justify-content-start">
                  {{$t('invoice.justneed')}}
                  <a @click="openLogin" href="javascript:;" style="margin-right: 5px;margin-left: 5px;color: #E6A23C;text-decoration: underline;font-size: 14px;">{{$t('invoice.complete')}}</a>
                  {{$t('invoice.then')}}
                  <a @click="openLogin" href="javascript:;" style="margin-right: 5px;margin-left: 5px;color: #E6A23C;text-decoration: underline;font-size: 14px;">{{$t('invoice.my')}}</a>
                  {{$t('invoice.historical')}}
                </p>
              </div>
              <div v-if="loginType ==1" class="tx-left" style="background-color: rgb(253, 246, 236);border-radius: 4px;border-left: 5px solid rgb(230, 162, 60);margin: 0 0 0 0px;height: 60px; padding: 0 20px;box-shadow: rgba(33, 33, 33, 0.4) 0px 1px 4px 0px;">
                <p style="color: #606266;font-weight: 600;margin-bottom: 0;font-size: 14px;line-height: 60px;" class="d-flex justify-content-start">
                  {{$t('invoice.just')}}
                  <a @click="openLogin" href="javascript:;" style="margin-right: 5px;margin-left: 5px;color: #E6A23C;text-decoration: underline;font-size: 14px;">{{$t('invoice.sign')}}</a>
                  {{$t('invoice.then')}}
                  <a @click="openLogin" href="javascript:;" style="margin-right: 5px;margin-left: 5px;color: #E6A23C;text-decoration: underline;font-size: 14px;">{{$t('invoice.my')}}</a>
                  {{$t('invoice.historical')}}
                </p>
        ...
        <div v-else-if="showTopTip && !fromPage">
          ...
          >
            <div style="width: 1100px;margin: 0 auto;padding: 0px 30px 0 0;">
              <div class="tx-left" style="background-color: #f0f9eb;border-radius: 4px;border-left: 5px solid #67C23A;margin: 0 0 0 0px;height: 60px; padding: 0 20px;box-shadow: rgba(33, 33, 33, 0.4) 0px 1px 4px 0px;">
                <p style="color: #606266;font-weight: 600;margin-bottom: 0;font-size: 14px;line-height: 60px;" class="d-flex justify-content-start">
                  {{$t('invoice.click')}}
                  <router-link :to="{path:'/dashboard'}" target="_blank" style="margin-right: 5px;margin-left: 5px;color: #67C23A;text-decoration: underline;font-size: 14px;">{{$t('invoice.my')}}</router-link>
                  {{$t('invoice.historical')}}
                </p>
              </div>
            </div>
      ...
      <div v-infinite-scroll="load" :infinite-scroll-distance="800" infinite-scroll-disabled="disabled">
          ...
          <el-main>
            <el-card shadow="always" style="margin-bottom: 20px;">
              <el-steps :active="stepActive" process-status="wait">
                <el-step :title="$t('invoice.step1')" :icon="stepActive > 1 ? 'el-icon-success' : ''" :description="$t('invoice.ImportShopify')"></el-step>
                <el-step :title="$t('invoice.step2')" :icon="stepActive > 2 ? 'el-icon-success' : ''" :description="$t('invoice.Improvenumber')"></el-step>
                <el-step :title="$t('invoice.step3')" :icon="stepActive > 3 ? 'el-icon-success' : ''" :description="$t('invoice.collectionstatus')"></el-step>
                <el-step :title="$t('invoice.step4')" :icon="stepActive > 4 ? 'el-icon-success' : ''" :description="$t('invoice.cooperation')"></el-step>
              </el-steps>
            </el-card>
            <template v-if="invoiceInfo.status != 4 && invoiceInfo.status != 5 && invoiceInfo.status != 6">
              <div v-if="invoiceInfo.extraJson && invoiceInfo.extraJson.importErrorLog && invoiceInfo.extraJson.importErrorLog.length" class="warning" style="padding: 8px 16px;
                background-color: #fff6f7;
                border-radius: 4px;
                border-left: 5px solid #fe6c6f;
                margin: 20px 0;">
                <div style="cursor: pointer;" @click="showErrorLog = !showErrorLog" class="d-flex align-items-center justify-content-between">
                  {{invoiceInfo.extraJson.importErrorLog[0]}}
                  <i v-show="invoiceInfo.extraJson.importErrorLog.length>1" :class="showErrorLog ? 'el-icon-arrow-down' : 'el-icon-arrow-up'"></i>
                </div>
                <div v-show="showErrorLog && invoiceInfo.extraJson.importErrorLog.length>1">
                  <p v-for="(log,i) in invoiceInfo.extraJson.importErrorLog" v-if="i != 0">
                    {{log}}
                  </p>
                </div>
              </div>
            ...
            <el-card shadow="always" class="m-card">
              <div v-if="invoiceInfo.status == '9'" class="cancel-label">{{$t('invoice.cancel')}}</div>
                ...
                <el-col :span="12">
                  <div class="grid-content invoice-header-info">
                    <div class="wv-heading-title">
                      {{Number(realAmount) > 0 ? 'INVOICE' : 'Purchase Order'}}
                    </div>
                    <template v-if="invoiceInfo.vendorName">
                      <div class="wv-text-strong">
                        {{invoiceInfo.vendorName?invoiceInfo.vendorName:'---'}}
                        <el-button v-if="invoiceInfo.payStatus == 2 && invoiceInfo.status != '9'" style="padding: 0;" slot="reference" type="text" @click="editCompany">{{$t('invoice.edit')}}</el-button>
                      </div>
                    </template>
                    <template v-else>
                      <el-input @change="editCompanyByInput()" class="tx-right-input" style="margin: 10px 0;" size="small" :placeholder="$t('invoice.fillname')" v-model="editCompanyName"></el-input>
                    </template>
                  </div>
                </el-col>
              </el-row>
              ...
              <div class="table-fixed">
                <div class="tabs d-flex">
                  <div class="tab-btn" @click="tabcChange(1)" :class="tabActive == 1 || tabActive == 2 ? 'active' : ''">
                    {{$t('invoice.Shopifyvalid')}}({{invoiceCnt.orderCnt}})
                  </div>

                  <div style="display: none;" class="tab-btn" @click="tabcChange(3)" :class="tabActive == 3 ? 'active' : ''">
                    <span slot="label"><i class="fa fa-exclamation-triangle" style="color: rgb(245, 108, 108);"></i> {{$t('invoice.SKUmissing')}}({{invoiceCnt.notBindCnt}})</span>
                  </div>

                  <div class="tab-btn" @click="tabcChange(4)" :class="tabActive == 4 ? 'active' : ''">
                    <span slot="label"><i class="iconfont icon-tubiaozhizuomoban" style="font-size: 18px;color: rgb(230, 162, 60);"></i> {{$t('invoice.Notshipped')}}({{invoiceCnt.notDeliverCnt > 99 ? '99+' : invoiceCnt.notDeliverCnt}})</span>
                  </div>
                  <div class="tab-btn" @click="tabcChange(8)" :class="tabActive == 8 ? 'active' : ''">
                    <span slot="label"><i class="fa fa-plane" style="font-size: 18px;color: #67C23A;"></i> {{$t('invoice.Delivered')}}({{invoiceCnt.deliverCnt > 99 ? '99+' : invoiceCnt.deliverCnt}})</span>
                  </div>
                  <div class="tab-btn" @click="tabcChange(5)" :class="tabActive == 5 ? 'active' : ''">
                    <span slot="label"><i class="fa fa-commenting" style="color: #5C6AC4;"></i> {{$t('invoice.Unreadmessage')}}({{invoiceCnt.notReadMessage > 99 ? '99+' : invoiceCnt.notReadMessage}})</span>
                  </div>
                  <div class="pos-absolute">

                    <el-button-group class="text-btn">
                      <el-button :class="showType == 1 ? 'active':''" @click="changeShowType('1')" size="mini" type="text">
                        <b class="radio-span"></b>
                        <i class="iconfont icon-shuzhuangtu"></i>
                        <span>{{$t('invoice.Ordergroup')}}</span>
                      </el-button>
                      <el-button :class="showType == 2 ? 'active':''" @click="changeShowType('2')" size="mini" type="text">
                        <b class="radio-span"></b>
                    ...
                    <template v-if="invoiceCnt.notBindCnt > 0">
                      ...
                      <el-tooltip placement="bottom">
                        ...
                        <el-button v-if="invoiceInfo.status != '9'" class="with-disabled" size="mini" type="success" icon="fa fa-send">{{$t('invoice.Sendinvoice')}}</el-button>
                      </el-tooltip>
                    </template>
                    <template v-else-if="invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2">
                      <el-popover ref="setPaymentType" popper-class="with-shadow" width="400" placement="top" :title="$t('invoice.paymentmethod')" trigger="hover">
                        ...
                        <el-button slot="reference" size="mini" type="success" icon="fa fa-send">{{$t('invoice.Sendinvoice')}}</el-button>
                      </el-popover>
                ...
                <div class="table-wrap" ref="orderItemsTable">
                  <template v-if="tabActive == 1">
                    <el-tooltip placement="left">
                      <div slot="content">{{$t('invoice.SKUmissing')}}({{invoiceCnt.notBindCnt}})</div>
                      <div class="tab-btn" v-show="invoiceCnt.notBindCnt > 0" @click="tabcChange(3)" style="cursor: pointer;">
                        <i class="fa fa-exclamation-triangle fa-2x"></i>
                        <span>{{invoiceCnt.notBindCnt > 99 ? '99+' : invoiceCnt.notBindCnt}}</span>
                      </div>
                    </el-tooltip>
                  </template>
                  <template v-if="tabActive == 2">
                    <el-tooltip placement="left">
                      <div slot="content">{{$t('invoice.SKUmissing')}}({{invoiceCnt.groupSkuCnt}})</div>
                      <div class="tab-btn" v-show="invoiceCnt.notBindCnt > 0" @click="tabcChange(6)" style="cursor: pointer;">
                        <i class="fa fa-exclamation-triangle fa-2x"></i>
                        <span>{{invoiceCnt.groupSkuCnt > 99 ? '99+' : invoiceCnt.groupSkuCnt}}</span>
                      </div>
                    </el-tooltip>
                  </template>
                  ...
                  <div class="table1" v-if="tabActive != 2">
                    <el-table :header-cell-style="getRowClass" :cell-class-name="getCellClass" :cell-style="cellStyle" class="table-with-body">
                      <el-table-column width="180">
                        <template slot="header">
                          <div class="d-flex align-item-center change-type" v-if="tabActive == 1">
                      ...
                      <el-table-column>
                        <template slot="header">
                          ...
                          <div v-if="searchName || searchTrack" class="d-flex">
                            <el-input size="mini" type="text" :placeholder="searchName ? $t('invoice.Supportsearchname') : $t('invoice.Supportsearchnumber')" v-model="searchNameInput" ref="searchNameInput" clearable @clear="Filter(2,'name')" @blur="Filter(3,'name')" @keyup.enter.native="Filter(1,'name')">
                      ...
                      <el-table-column prop="price" :label="$t('invoice.Supplyunit')" width="120">
                      </el-table-column>
                  ...
                  <div class="table2" v-if="tabActive == 2">
                    <el-table :header-cell-style="getRowClass3" :cell-class-name="getCellClass" :cell-style="cellStyle" class="table-with-body">
                      ...
                      <el-table-column prop="price" :label="$t('invoice.Supplyunit')" width="200">
                      </el-table-column>
                      <el-table-column :label="$t('invoice.Totalsupply')" width="120">
                      </el-table-column>
                    </el-table>
              ...
              <template v-if="tabActive != 5">
                <div class="table-wrap">
                  <template v-if="invoiceData.length">
                    ...
                    <el-table :header-cell-style="getRowClass" :cell-style="cellStyle" class="table-only table-only-top">
                      <el-table-column width="150" :label="$t('invoice.SupplierSKU')">
                      </el-table-column>
                        ...
                        <el-table-column width="180">
                          ...
                          <template v-slot="scope">
                            <div class="d-flex sku-with-error">
                              <el-tooltip placement="left" v-if="(!scope.row.sku || !scope.row.price) && scope.row.quantity > 0 && scope.row.fulfillStatus != 2">
                                <div slot="content">{{$t('invoice.SKUmissing')}}<br />{{$t('invoice.Reason')}}
                                  <span v-if="!scope.row.sku && !scope.row.price">{{$t('invoice.pricemissing')}}</span>
                                  <span v-else-if="!scope.row.sku">{{$t('invoice.SKUmissing')}}</span>
                                  <span v-else="!scope.row.price">{{$t('invoice.supplypricemissing')}}</span>
                                </div>
                                <i class="fa fa-exclamation-triangle mg-r-10" style="color: #F56C6C;"></i>
                              </el-tooltip>
                              <div>
                                <div v-if="isChangeSku(scope.row)">
                                  <el-input
                                    ...
                                    @change="changeSkuFn(scope.row,scope.$index)"
                                  ></el-input>
                                </div>
                                <template v-else>
                                  <el-popover popper-class="with-shadow" width="400" :ref="`popoverSku-${scope.$index}`" placement="top" :title="$t('invoice.Batchset')" trigger="manual">

                                    <div>
                                      <div class="mg-t-20">
                                        <a href="javascript:;" class="input-btn d-flex" :class="form.resource == 1 ? 'active' : ''" style="color: #333;" @click="setResource(1)">
                                          <span class="mg-r-5"></span> {{$t('invoice.samekind')}}
                                          <p style="display: inline;margin-left: 5px;" v-show="scope.row.noneCntSku" v-html="$t('invoice.hereare',{num: Number(scope.row.noneCntSku)})"></p>
                                        </a>
                                      </div>
                                      <div class="mg-t-10">
                                        <a href="javascript:;" class="input-btn d-flex" :class="form.resource == 2 ? 'active' : ''" style="color: #333;" @click="setResource(2)">
                                          <span class="mg-r-5"></span> {{$t('invoice.Allsame')}}
                                          <p style="display: inline;margin-left: 5px;" v-show="scope.row.sameCntSku" v-html="$t('invoice.hereare',{num: Number(scope.row.sameCntSku)})"></p>
                                        </a>
                                      </div>
                                      <div class="mg-t-10">
                                        <div class="d-flex justify-content-between">
                                          <span class="">{{$t('invoice.Batchsetto')}} <span class="color-primary tx-600">{{scope.row.sku}}</span></span>
                                          <div class="tx-right">
                                            <el-button size="small" @click="scope._self.$refs[`popoverSku-${scope.$index}`].doClose()">{{$t('invoice.cancel')}}</el-button>
                                            <el-button size="small" type="primary" @click="changeSkuBatch(scope.row)">{{$t('invoice.Save1')}}</el-button>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                    <el-input slot="reference" size="small" placeholder="SKU" v-model="scope.row.sku" @keyup.native="getSkuInfo(scope.row,scope.$index)" @change="changeSkuFn(scope.row,scope.$index)">
                                    </el-input>
                                  </el-popover>
                                </template>
                              </div>
                            </div>
                            <div v-if="invoiceInfo.payStatus != 1 && invoiceInfo.customerRead != 1" style="margin-top: 0;">
                              <template v-if="scope.row.fulfillStatus == 1">
                                <el-popover :ref="`popover-stop-${scope.$index}`"  width="400" placement="top" :title="$t('invoice.deleteReason')" trigger="click">
                                  <div>
                                    <div class="mg-t-20">
                                      <a href="javascript:;" class="input-btn d-flex" :class="stopResource == 1 ? 'active' : ''" style="color: #333;" @click="setStopResource(1)">
                                        <span class="mg-r-5"></span> {{$t('invoice.Outstock')}}
                                      </a>
                                    </div>
                                    <div class="mg-t-10">
                                      <a href="javascript:;" class="input-btn d-flex" :class="stopResource === 2 ? 'active' : ''" style="color: #333;" @click="setStopResource(2)">
                                        <span class="mg-r-5"></span> 暂时移除
                                        <el-tooltip content="表示从当前采购单中暂时移除，稍后可以继续对此Shopify订单生成采购订单。" placement="right">
                                          <i class="el-icon-info" style="margin-left: 2px;font-size: 14px;color: #5c6ac4;"></i>
                                        </el-tooltip>
                                      </a>
                                    </div>
                                    <div class="mg-t-10">
                                      <a href="javascript:;" class="input-btn d-flex" :class="stopResource === 3 ? 'active' : ''" style="color: #333;" @click="setStopResource(3)">
                                        <span class="mg-r-5"></span> 尚未合作
                                        <el-tooltip placement="right">
                                          <div slot="content">表示该商品您无法供货，之后再新生成采购订单时，该商品将自动设为不发货<br/>（如您以后可以供货，可在“尚未合作商品”列表中对该商品进行报价和处理）</div>
                                          <i class="el-icon-info" style="margin-left: 2px;font-size: 14px;color: #5c6ac4;"></i>
                                        </el-tooltip>
                                      </a>
                                    </div>
                                    <div class="mg-t-10">
                                      <a href="javascript:;" class="input-btn d-flex" :class="stopResource == 99 ? 'active' : ''" style="color: #333;" @click="setStopResource(99)">
                                        <span class="mg-r-5"></span> {{$t('invoice.Other')}}
                                      </a>
                                    </div>

                                    <div class="mg-t-10">
                                      <div class="tx-right">
                                        <el-button size="small" @click="scope._self.$refs[`popover-stop-${scope.$index}`].doClose()">{{$t('invoice.cancel')}}</el-button>
                                        <el-button size="small" type="primary" @click="setStop(scope.row,'2')">{{$t('invoice.confirm')}}</el-button>
                                      </div>
                                    </div>
                                  </div>
                                  <el-link
                                  ...
                                  >
                                    <i style="margin-right: 3px;" class="el-icon-remove-outline"></i>
                                    {{$t('invoice.Setdelete')}}
                                  </el-link>
                                </el-popover>
                              </template>
                              <template v-else-if="invoiceInfo.allowWithdraw == 1">
                                <el-button @click="setStop(scope.row,'1')" type="text" style="color: #67C23A;font-size: 12px;">{{$t('invoice.Resume')}}</el-button>
                              </template>
                            </div>
                          </template>
                        </el-table-column>
                        ...
                        <el-table-column>
                          <template #header>
                            ...
                            <div v-if="searchName || searchTrack" class="d-flex">
                              <el-input
                                size="mini"
                                type="text"
                                :placeholder="searchName ? $t('invoice.Supportsearchname') : $t('invoice.Supportsearchnumber')"
                          ...
                          <template v-slot="scope">
                            <div style="display: flex;align-items: flex-start;">
                              <el-image style="min-width: 40px;width: 40px;height: 40px;margin: 5px 5px 0 0;border: 1px solid #ddd;" lazy :src="scope.row.imgUrl" :preview-src-list="scope.row.previewList">
                                <div slot="error" class="image-slot">
                                  <i class="el-icon-picture-outline" style="font-size: 20px;"></i>
                                </div>
                              </el-image>
                              <div style="max-width: 100%; text-align: left;flex: 100%;">
                                <div class="tx-600">
                                  {{scope.row.name}}
                                  <template v-if="scope.row.customerProperties && scope.row.customerProperties.length">
                                    <div v-for="val in scope.row.customerProperties" :key="val.name" class="d-flex">
                                      <div class="d-flex" style="width: 90%;">
                                        <div style="text-align: right;color: #999;" class="">{{val.name}}: </div>
                                ...
                                <div class="d-flex no-wrap justify-content-between" v-if="tabActive != 2">
                                  <div class="d-flex">
                                    <el-popover
                                      ...
                                      trigger="hover"
                                    >
                                      <div slot="reference" style="cursor: pointer;display: flex; max-width: 280px;">
                                        <span style="margin-right: 10px;">{{scope.row.codeName}}<i class="el-icon-caret-bottom el-icon--right"></i></span>
                                      </div>

                                      <div class="mg-b-20 mg-t-10">
                                        <div style="justify-content: space-between;" class="d-flex mg-b-20">
                                          <div style="font-weight: bold;font-size: 16px" class="title">{{$t('invoice.ShopifyOrders')}} {{scope.row.codeName}}</div>
                                          <div>
                                            <span style="color: #999;" class="">{{$t('invoice.Orderdate')}}</span>
                                            <span class="mg-l-5 pd-r-15">{{scope.row.timeCreated?scope.row.timeCreated:'---'}}</span>
                                          </div>
                                        </div>
                                        <div class="mg-y-10" style="font-weight: bold;font-size: 16px" >收件信息</div>
                                        <div class="d-flex">
                                          <div class="d-flex" style="width: 90%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">收件人: </div>
                                            ...
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.name">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.name" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        </div>
                                        <div class="d-flex">
                                          <div class="d-flex" style="width: 90%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">收货地址1: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.address1 ? scope.row.shippingJson.address1 : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.address1">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.address1" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        </div>
                                        <div class="d-flex">
                                          <div class="d-flex" style="width: 90%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">收货地址2: </div>
                                            <div class="mg-l-5 pd-r-15" style="max-width:75%;">
<!--                                              {{scope.row.shippingJson && scope.row.shippingJson.zip ? scope.row.shippingJson.zip : '-&#45;&#45;'}}-->
                                              {{scope.row.shippingJson && scope.row.shippingJson.address2 ? scope.row.shippingJson.address2 : '---'}}
...
<!--                                              {{scope.row.shippingJson && scope.row.shippingJson.province ? scope.row.shippingJson.province : '-&#45;&#45;'}}-->
                                            ...
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.address2">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.address2" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        </div>
                                        <div class="d-flex">
                                          <div class="d-flex" style="width: 50%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">城市: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.city ? scope.row.shippingJson.city : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.city">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.city" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                          <div class="d-flex" style="width: 50%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">省份: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.province ? scope.row.shippingJson.province : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.province">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.city" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        ...
                                        <div class="d-flex" style="justify-content: space-between">
                                          <div class="d-flex" style="flex: 0 0 50%">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">国家: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.country ? scope.row.shippingJson.country : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.country">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.country" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                          <div class="d-flex" style="flex: 0 0 50%">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">邮编: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.zip ? scope.row.shippingJson.zip : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson && scope.row.shippingJson.zip">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.zip" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        </div>
                                        ...
                                        <div class="d-flex">
                                          <div class="d-flex" style="width: 90%;">
                                            <div style="width: 100px;text-align: right;color: #999;" class="">联系电话: </div>
                                            <div class="mg-l-5 pd-r-15">
                                              {{scope.row.shippingJson && scope.row.shippingJson.phone ? scope.row.shippingJson.phone : '---'}}
                                            </div>
                                            <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                              <el-button size="mini" type="text" v-if="scope.row.shippingJson.phone">
                                                <i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.phone" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                              </el-button>
                                            </el-tooltip>
                                          </div>
                                        </div>
                                        <template v-if="scope.row.note">
                                          <div class="mg-y-10" style="font-weight: bold;font-size: 16px" >订单备注</div>
                                          <div class="d-flex">
                                            <div class="d-flex" style="width: 90%;">
                                              <div style="width: 100px;text-align: right;color: #999;" class="">备注: </div>
                                              <div class="mg-l-5 pd-r-15">
                                               {{scope.row.note}}
                                              </div>
                                              <el-tooltip class="item" effect="dark" content="copy" placement="left">
                                                <el-button size="mini" type="text" v-if="scope.row.note">
                                                  <i class="el-icon-document-copy" v-clipboard:copy="scope.row.note" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                                                </el-button>
                                              </el-tooltip>
                                            </div>
                                          </div>
                                    ...
                                    <el-tooltip v-if="scope.row.shippingJson && scope.row.shippingJson.name" class="item" effect="dark" :content="scope.row.shippingJson.name" placement="top">
                                      <el-button size="mini">To</el-button>
                                    </el-tooltip>
                                    <div class="mg-l-5 mg-r-5">{{scope.row.shippingJson && scope.row.shippingJson.country ? scope.row.shippingJson.country : '---'}}</div>
                                  ...
                                  <div class="no-wrap d-flex">
                                    <div v-if="scope.row.trackNumber" class="d-flex">
                                      ...
                                      <div>
                                        <el-tooltip v-if="scope.row.trackCompany && scope.row.realCompany" class="item" effect="dark" placement="top">
                                          <div slot="content">
                                            <p style="margin-bottom: 5px;">{{scope.row.trackCompany}}({{scope.row.realCompany}})</p>
                                            {{scope.row.trackNumber}}
                                          </div>
                                          <i class="fa fa-truck"></i>
                                        </el-tooltip>
                                        <el-tooltip v-else-if="scope.row.trackCompany" class="item" effect="dark" placement="top">
                                          <div slot="content">
                                            <p style="margin-bottom: 5px;">{{scope.row.trackCompany}}</p>
                                            {{scope.row.trackNumber}}
                                          </div>
                                          <i class="fa fa-truck"></i>
                                        </el-tooltip>
                                        <el-tooltip v-else class="item" effect="dark" placement="top">
                                          <div slot="content">
                                            <p style="margin-bottom: 5px;">{{scope.row.realCompany}}</p>
                                            {{scope.row.trackNumber}}
                                          </div>
                                          <i class="fa fa-truck"></i>
                                        </el-tooltip>
                                      ...
                                      <el-tooltip v-if="scope.row.syncStatus == 2" class="item" effect="dark" placement="right">
                                        <div slot="content">{{$t('invoice.deliveredgoods')}}
                                          <br/>{{$t('invoice.Reason')}} {{invoiceInfo.hasAuth != 1 ? $t('invoice.notauthorized') : scope.row.syncContent}}</div>

                                        <i class="invoice-icon-tongbu1"></i>
                                      </el-tooltip>
                                      <el-tooltip v-else class="item" effect="dark" :content="$t('invoice.hasbeensync')" placement="right">
                                    ...
                                    <div v-else-if="scope.row.fulfillStatus == 1">
                                      <el-tooltip class="item" effect="dark" :content="$t('invoice.hasnot')" placement="right">
                                        <el-alert :title="$t('invoice.Notshipped')" type="warning" :closable="false">
                                        </el-alert>
                                      </el-tooltip>
                                    </div>
                                    <div v-else>
                                      <el-tooltip class="item" effect="dark" :content="$t('invoice.wontReason') + scope.row.stopReason" placement="right">
                                        <el-alert :title="$t('invoice.wontdeliver')" type="error" :closable="false">
                                        </el-alert>
                                      </el-tooltip>
                                    </div>
                                    <a style="margin-left: 5px;" v-if="invoiceInfo.payStatus == '1' && invoiceInfo.status != '9' && !scope.row.trackNumber && scope.row.fulfillStatus != 2" href="javascript:;" @click="markShippedOpen(scope.row)">{{$t('invoice.shippedMark')}}</a>
                                  </div>
                                </div>
                              </div>
                            </div>
                        ...
                        <el-table-column prop="qty" width="160">
                          <template #header>
                            {{$t('invoice.Purchasequantity')}}
                            <el-popover v-if="totalCnt" trigger="hover" placement="right">
                              <div slot v-loading="loadCnt" style=" min-height: 50px; font-size: 12px;">
                                <div style="margin-bottom: 5px;">{{$t('invoice.Totalpurchases')}}<b>{{totalCnt}}</b> 个</div>
                                <template v-if="cnt">
                                  ...
                                  <div style="text-align: right;">
                                    <a href="javascript:;" style="color: #5c6ac4;" v-clipboard:copy="cntText" v-clipboard:success="cntOnCopy" v-clipboard:error="cntOnError">{{$t('invoice.copy')}}</a>
                                  </div>
                                </template>
                              </div>
                              <span slot="reference" @mouseenter="getCntCountry">
															(<span style="text-decoration: underline; cursor: pointer;">{{totalCnt}}</span>)
														</span>
                            </el-popover>
                          </template>
                          <template slot-scope="scope">
                            <template v-if="isChangeOther(scope.row)">
                              <el-input-number :disabled="true" size="small" :min="1" v-model="scope.row.quantity" @change="handleChange(scope.row,$event)"></el-input-number>
                            </template>
                            <template v-else>
                              <el-input-number size="small" :min="1" v-model="scope.row.quantity" @change="handleChange(scope.row,$event)"></el-input-number>
                            </template>
                            <el-tooltip class="item" effect="dark" :content="$t('invoice.Abnormalreasons') + scope.row.abnormalReason" placement="right">
														<span v-if="scope.row.isAbnormal == 1" class="tx-danger" style="margin-top: 10px;display: inline-block;">
                              <i class="el-icon-info"></i>{{$t('invoice.Abnormal')}}
                            </span>
                            </el-tooltip>
                          </template>
                        </el-table-column>
                        <!-- 采购数量 end -->
                        <!-- 供货单价 start -->
                        <el-table-column prop="price" label="" :width="!isChangeAmount() ? '160px' : '120px'">
                          <template #header>
                            {{$t('invoice.Supplyunit')}}
                            <a v-if="!isChangeAmount() && !tempCode" @click="setLotPrice()" href="javascript:;" class="mg-l-0" style="font-weight: bold;">{{$t('invoice.Batchset')}}</a>
                            <span v-if="tempCode" style="color: #E6A23C">(设价中)</span>
                          </template>
                          <template slot-scope="scope">
                            <div>
                              <div v-if="isChangePrice(scope.row)">
                                <el-input disabled="disabled" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" v-model="scope.row.price" @change="changePriceFn(scope.row,scope.$index)">
                                </el-input>
                              </div>
                              <template v-else>
                                <el-popover popper-class="with-shadow" width="400" :ref="`popoverPrice-${scope.$index}`" placement="top" :title="$t('invoice.Batchsupplyset')" trigger="manual">

                                  <div>
                                    <div class="mg-t-20">
                                      <a href="javascript:;" class="input-btn d-flex" :class="formPrice.resource == 1 ? 'active' : ''" style="color: #333;" @click="setPriceResource(1)">
                                        <span class="mg-r-5"></span> {{$t('invoice.Allproducts')}}
                                        <p style="display: inline;margin-left: 5px;" v-show="scope.row.noneCntPrice" v-html="$t('invoice.hereare',{num: Number(scope.row.noneCntPrice)})"></p>
                                      </a>
                                    </div>
                                    <div class="mg-t-10">
                                      <a href="javascript:;" class="input-btn d-flex" :class="formPrice.resource == 2 ? 'active' : ''" style="color: #333;" @click="setPriceResource(2)">
                                        <span class="mg-r-5"></span> {{$t('invoice.Allsame')}}
                                        <p style="display: inline;margin-left: 5px;" v-show="scope.row.sameCntPrice" v-html="$t('invoice.hereare',{num: Number(scope.row.sameCntPrice)})"></p>
                                      </a>
                                    </div>
                                    <div class="mg-t-10">
                                      <div class="d-flex justify-content-between">
                                        <span class="">{{$t('invoice.Batchsetto')}} <span class="color-primary tx-600">{{scope.row.price}}</span></span>
                                        <div class="tx-right">
                                          <el-button size="small" @click="scope._self.$refs[`popoverPrice-${scope.$index}`].doClose()">{{$t('invoice.cancel')}}</el-button>
                                          <el-button size="small" type="primary" @click="changePriceBatch(scope.row)">{{$t('invoice.Save1')}}</el-button>
                                        </div>

                                      </div>
                                    </div>
                                  </div>
                                  <el-input type="number" @input="scope.row.price = (scope.row.price.match(/^\d*(\.?\d{0,2})/g)[0]) || null" slot="reference" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" v-model="scope.row.price" @keyup.native="getPriceInfo(scope.row,scope.$index)" @change="changePriceFn(scope.row,scope.$index)">
                                  </el-input>
                                </el-popover>
                              </template>
                            </div>
                    ...
                    <div v-if="tabActive == 2">
                      ...
                      >
                            ...
                            <el-card shadow="never">
                              <div>
                                <el-table
                                  ...
                                  :data="props.row.allItems"
                                  stripe
                                  border
                                  class="table-with-body"
                                >
                                  <el-table-column width="150" prop="sku" :label="$t('invoice.SupplierSKU')">
                                    <template slot-scope="scope">
                                      <div class="d-flex sku-with-error">
                                        <el-tooltip placement="left" v-if="(!scope.row.sku || !scope.row.price) && scope.row.quantity > 0 && scope.row.fulfillStatus != 2">
                                          <div slot="content">{{$t('invoice.SKUmissing')}}<br />{{$t('invoice.Reason')}}
                                          ...
                                          <i class="fa fa-exclamation-triangle mg-r-10" style="color: #F56C6C;left: -55px;"></i>
                                        </el-tooltip>

                                        <div>
                                          <div v-if="isChangeSku(scope.row)">
                                            <el-input disabled="disabled" size="small" placeholder="SKU" v-model="scope.row.sku" @change="changeSkuFnBySub(scope.row)">
                                            </el-input>
                                          </div>
                                          <template v-else>
                                            <el-input slot="reference" size="small" placeholder="SKU" v-model="scope.row.sku" @change="changeSkuFnBySub(scope.row)">
                                            </el-input>
                                          </template>
                                        </div>
                                      </div>
                                      ...
                                      <div>
                                        <div style="max-width: 100%; text-align: left;">
                                          <div class="d-flex no-wrap justify-content-between">
                                            <el-popover popper-class="customer-popover" placement="bottom" :title="`${$t('invoice.ShopifyOrders')} ${scope.row.codeName}`" width="500" trigger="hover">
                                              <div slot="reference" style="cursor: pointer;">
                                                <span style="margin-right: 10px;">{{scope.row.codeName}}<i class="el-icon-caret-bottom el-icon--right"></i></span>
                                                <span style="display: inline-block;max-width: 100px;text-overflow: ellipsis;" v-if="scope.row.shippingJson && scope.row.shippingJson.name">To {{scope.row.shippingJson.name}} </span>
                                                <span style="display: inline-block;max-width: 100px;text-overflow: ellipsis;" v-if="scope.row.shippingJson && scope.row.shippingJson.country"> ({{scope.row.shippingJson.country}})</span>
                                              </div>
                                              <div class="mg-y-15 mg-t-20">
                                                <div class="d-flex">
                                                  <div class="d-flex" style="width: 90%;">
                                                    <div style="width: 100px;text-align: right;color: #999;" class="">{{$t('invoice.Orderdate')}}</div>
                                                    <div class="mg-l-5 pd-r-15">{{scope.row.timeCreated}}</div>
                                                  </div>
                                                </div>
                                                <div class="d-flex">
                                                  <div class="d-flex" style="width: 90%;">
                                                    <div style="width: 100px;text-align: right;color: #999;" class="">{{$t('invoice.Buyer')}}</div>
                                                    <div class="mg-l-5 pd-r-15">{{scope.row.shippingJson && scope.row.shippingJson.name ? scope.row.shippingJson.name : '---'}}</div>
                                                  </div>
                                                  <el-tooltip class="item" effect="dark" :content="$t('invoice.Copyinformation')" placement="left">
                                                    <el-button size="mini" type="text"><i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.name" v-clipboard:success="onCopy" v-clipboard:error="onError"></i></el-button>
                                                  </el-tooltip>
                                                </div>
                                                <div class="d-flex">
                                                  <div class="d-flex" style="width: 90%;">
                                                    <div style="width: 100px;text-align: right;color: #999;" class="">{{$t('invoice.address')}}</div>
                                                    <div class="mg-l-5 pd-r-15" style="max-width:75%;">
                                                      {{scope.row.shippingJson && scope.row.shippingJson.zip ? scope.row.shippingJson.zip : '---'}} {{scope.row.shippingJson && scope.row.shippingJson.address1 ? scope.row.shippingJson.address1 : '---'}} {{scope.row.shippingJson && scope.row.shippingJson.city ? scope.row.shippingJson.city : '---'}} {{scope.row.shippingJson && scope.row.shippingJson.province ? scope.row.shippingJson.province : '---'}}
                                                    </div>
                                                  </div>
                                                </div>
                                                <div class="d-flex">
                                                  ...
                                                  <el-tooltip class="item" effect="dark" :content="$t('invoice.Copyaddress')" placement="left">
                                                    <el-button size="mini" type="text"><i class="el-icon-document-copy" v-clipboard:copy="scope.row.shippingJson.zip+scope.row.shippingJson.address1+scope.row.shippingJson.city+scope.row.shippingJson.province+scope.row.shippingJson.country" v-clipboard:success="onCopy" v-clipboard:error="onError"></i></el-button>
                                                  </el-tooltip>
                                                </div>
                                                <div class="d-flex">
                                                  <div class="d-flex" style="width: 90%;">
                                                    <div style="width: 100px;text-align: right;color: #999;" class="">{{$t('invoice.BuyerEmail')}}</div>
                                                    <div class="mg-l-5 pd-r-15">{{scope.row.email}}</div>
                                                  </div>
                                                  <template v-if="scope.row.email">
                                                    <el-tooltip class="item" effect="dark" :content="$t('invoice.CopyEmail')" placement="left">
                                                      <el-button size="mini" type="text"><i class="el-icon-document-copy" v-clipboard:copy="scope.row.email" v-clipboard:success="onCopy" v-clipboard:error="onError"></i></el-button>
                                                    </el-tooltip>
                                                  </template>
                                                </div>
                                              </div>
                                  ...
                                  <el-table-column prop="qty" width="160">
                                    ...
                                    <template slot-scope="scope">
                                      <template>
                                        <template v-if="isChangeOther(scope.row)">
                                          <el-input-number :disabled="true" size="small" :min="1" v-model="scope.row.quantity" @change="handleChangeBySub(props,scope.row,$event)"></el-input-number>
                                        </template>
                                        <template v-else>
                                          <el-input-number size="small" :min="1" v-model="scope.row.quantity" @change="handleChangeBySub(props,scope.row,$event)"></el-input-number>
                                        </template>
                                      </template>
                                    </template>
                                  </el-table-column>
                                  <el-table-column prop="price" :label="$t('invoice.Supplyunit')" width="120">
                                    <template slot-scope="scope">
                                      <div>
                                        <div v-if="isChangePrice(scope.row)">
                                          <el-input disabled="disabled" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" v-model="scope.row.price" @change="changePriceFnBySub(props,scope.row)">
                                          </el-input>
                                        </div>
                                        <template v-else>
                                          <el-input-number slot="reference" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" :min="0" :controls="false" :precision="2" v-model="scope.row.price" @change="changePriceFnBySub(props,scope.row)">
                                          </el-input-number>
                                        </template>
                                      </div>
                                    </template>
                                  ...
                                  <el-table-column :label="$t('invoice.Totalsupply')" width="120">
                                    <template slot-scope="scope">
                                      <div v-if="isChangePrice(scope.row)">
                                        <el-input name="price-input" disabled="disabled" size="small" :placeholder="$t('invoice.Totalsupply')" v-model="scope.row.amount">
                                        </el-input>
                                      </div>
                                      <template v-else>
                                        <el-input name="price-input" type="number" @input="scope.row.amount = (scope.row.amount.match(/^\d*(\.?\d{0,2})/g)[0]) || null" size="small" :placeholder="$t('invoice.Totalsupply')" v-model="scope.row.amount" @change="changePriceTotalFnBySub(props,scope.row)">
                                        </el-input>
                                      </template>
                                      <!--<div>
																			{{$numberToCurrency(scope.row.amount)}}
                        ...
                        <el-table-column width="180">
                          ...
                          <template v-slot="scope">
                            <div class="d-flex sku-with-error">
                              ...
                              <div>
                                <div v-if="isChangeSku(scope.row)">
                                  <el-input disabled="disabled" size="small" placeholder="SKU" v-model="scope.row.sku" @change="changeSkuFn(scope.row,scope.$index)">
                                  </el-input>
                                </div>
                                <template v-else>
                                  <el-popover popper-class="with-shadow" width="400" :ref="`popoverSkuGroup-${scope.$index}`" placement="top" :title="$t('invoice.Batchset')" trigger="manual">
                                    <div>
                                      <div class="mg-t-10">
                                        <div class="d-flex justify-content-between">
																				<span class="">{{$t('invoice.Total')}}
                                          <span class="color-primary tx-600">{{scope.row.cnt}}</span>{{$t('invoice.haveset')}}
                                          <span class="color-primary tx-600">{{scope.row.sku}}</span>
																				</span>
                                        </div>
                                      </div>
                                      <div class="mg-t-10">
                                        <div class="tx-right">
                                          <el-button size="small" @click="scope._self.$refs[`popoverSkuGroup-${scope.$index}`].doClose()">{{$t('invoice.cancel')}}</el-button>
                                          <el-button size="small" type="primary" @click="changeSkuFn(scope.row,scope.$index,'close')">{{$t('invoice.Save1')}}</el-button>
                                        </div>
                                      </div>
                                    </div>
                                    <el-input slot="reference" size="small" placeholder="SKU" v-model="scope.row.sku" @keyup.native="getSkuInfo2(scope.row,scope.$index,'popoverSkuGroup')" @change="changeSkuFn(scope.row,scope.$index)">
                                    </el-input>
                                  </el-popover>
                                  <!-- 批量设为不发货 -->
                                  ...
                                  >
                                    <div>
                                      ...
                                      <div class="mg-t-10">
                                        <a href="javascript:;" class="input-btn d-flex" :class="stopResource === 3 ? 'active' : ''" style="color: #333;" @click="setStopResource(3)">
                                          <span class="mg-r-5"></span> 尚未合作
                                          <el-tooltip placement="right">
                                            <div slot="content">表示该商品您无法供货，之后再新生成采购订单时，该商品将自动设为不发货
                                    ...
                                    <el-link
                                      ...
                                      @click="showPopover3"
                                      :underline="false"
                                      style="color: #F56C6C;font-size: 12px;text-align: left;display: block"
                                    >
                                      <i style="margin-right: 3px;" class="el-icon-remove-outline sbns-icon"></i>
                                      {{$t('invoice.Setdelete')}}
                                    </el-link>
                                  </el-popover>
                        ...
                        <el-table-column>
                          ...
                          <template slot-scope="scope">
                            <div class="d-flex">
                              <el-image style="min-width: 40px;width: 40px;height: 40px;margin: 0 5px 0 0;border: 1px solid #ddd;" lazy :src="scope.row.imgUrl" :preview-src-list="scope.row.previewList">
                                <div slot="error" class="image-slot">
                                  <i class="el-icon-picture-outline" style="font-size: 20px;"></i>
                                </div>
                              </el-image>
                        ...
                        <!--  订单维度-供货单价 start   -->
                        <el-table-column prop="price" :label="$t('invoice.Supplyunit')" :width="!isChangeAmount()  ? '160px' : '120px'">
                          <template slot-scope="scope">
                            <div>
                              <div v-if="isChangePrice(scope.row)" class="d-flex">
                                <el-input disabled="disabled" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" v-model="scope.row.price" @change="changePriceFn(scope.row,scope.$index)">
                                </el-input>
                                <a v-if="!isChangeAmount() && scope.fulfillStatus ==='1'" @click.stop="setLotPrice(scope.row)" href="javascript:;" class="mg-l-0" style="font-weight: bold;white-space: nowrap;margin-left: 10px;">{{$t('invoice.Batchset')}}</a>
                              </div>
                              <div v-else class="d-flex">
                                <el-popover popper-class="with-shadow" width="400" :ref="`popoverPriceGroup-${scope.$index}`" placement="top" :title="$t('invoice.Batchsupplyset')" trigger="manual">
                                  <div>
                                    <div class="mg-t-10">
                                      <div class="d-flex justify-content-between">
																			<span class="" >
																				<span v-html="$t('invoice.hereare',{num: Number(scope.row.cnt)})"></span>{{$t('invoice.Batchsetto')}}
																				<span class="color-primary tx-600">{{scope.row.price}}</span>
																			</span>
                                      </div>
                                    </div>
                                    <div class="mg-t-10">
                                      <div class="tx-right">
                                        <el-button size="small" @click="scope._self.$refs[`popoverPriceGroup-${scope.$index}`].doClose()">{{$t('invoice.cancel')}}</el-button>
                                        <el-button size="small" type="primary" @click="changePriceFn(scope.row,scope.$index,'close')">{{$t('invoice.Save1')}}</el-button>
                                      </div>

                                    </div>
                                  </div>
                                  <el-input type="number" @input="scope.row.price = (scope.row.price.match(/^\d*(\.?\d{0,2})/g)[0]) || null" slot="reference" name="price-input" size="small" :placeholder="$t('invoice.Supplyunit')" v-model="scope.row.price" @keyup.native="getPriceInfo2(scope.row,scope.$index,'popoverPriceGroup')" @change="changePriceFn(scope.row,scope.$index)">
                                  </el-input>
                                </el-popover>
                                <a v-if="!isChangeAmount()" @click.stop="setLotPrice(scope.row)" href="javascript:;" class="mg-l-0" style="font-weight: bold;white-space: nowrap;margin-left: 10px;">{{$t('invoice.Batchset')}}</a>
                              </div>
                              <p v-show="scope.row.minPrice > 0 && scope.row.minPrice != scope.row.maxPrice">
                                {{$t('invoice.supplyprice')}}{{$numberToCurrency(scope.row.minPrice)}} ~ {{$numberToCurrency(scope.row.maxPrice)}}
                              </p>
                            </div>
                          </template>
                        </el-table-column>
                  ...
                  <template v-if="invoiceData.length == 0">
                    ...
                    <el-table :row-class-name="tableRowClassName" :header-cell-style="getRowClass" :cell-class-name="getCellClass" :cell-style="cellStyle" :data="invoiceData" stripe border class="table-with-body">
                ...
                <template v-if="tabActive == 1 || tabActive == 2">
                      ...
                      <el-col :span="8">
                        <div class="text item">
                          ...
                          <div style="background-color: #fff;padding: 20px;">
                            <p v-show="invoiceInfo.customerRead == 1" style="color: #67C23A;margin-bottom: 20px;">
                              <i class="el-icon-success" style="font-size: 16px;"></i>
                              <span style="display: inline-block;width: 120px;margin: 0 5px;">{{$t('invoice.Customerconfirmed')}}</span>{{invoiceInfo.timeRead ? invoiceInfo.timeRead : "---"}}
                            </p>
                            <p v-show="invoiceInfo.payStatus == 1" style="color: #67C23A;margin-bottom: 20px;">
                              <i class="el-icon-success" style="font-size: 16px;"></i>
                              <span style="display: inline-block;width: 120px;margin: 0 5px;">{{$t('invoice.Supplierconfirmed')}} </span>{{invoiceInfo.timePaid ? invoiceInfo.timePaid : "---"}}
                            </p>
                            <p v-show="invoiceInfo.timeApiGet" style="color: #67C23A;margin-bottom: 20px;">
                              <i class="el-icon-success" style="font-size: 16px;"></i>
                              <span style="display: inline-block;width: 120px;margin: 0 5px;">{{$t('invoice.ERPdownloaded')}} </span>{{invoiceInfo.timeApiGet ? invoiceInfo.timeApiGet : "---"}}
                            </p>
                            <p v-show="invoiceInfo.firstDeliverTime" style="color: #67C23A;margin-bottom: 20px;">
                              <i class="el-icon-success" style="font-size: 16px;"></i>
                              <span style="display: inline-block;width: 120px;margin: 0 5px;">{{$t('invoice.Supplierstarts')}}</span>{{invoiceInfo.firstDeliverTime ? invoiceInfo.firstDeliverTime : "---"}}
                            </p>
                          </div>
                        </div>
                      </el-col>
                      <el-col :span="16">
                        <div class="grid-content invoice-bottom">
                          <div class="d-flex justify-content-end mg-b-10">
                            <div class="">{{$t('invoice.Producttotal')}}(US$) :</div>
                            <div class="mg-l-5 pd-r-15">{{realAmount > 0 ? $numberToCurrency(itemAmount) : $t('invoice.Notset')}}</div>
                          </div>
                          <div class="d-flex justify-content-end">
                            <div class="">{{$t('invoice.Shippingtotal')}} (US$) :</div>
                            <div class="mg-l-5">
                              <el-input :disabled="isChangeAmount()" name="price-input" size="small" v-model="shippingAmount" :placeholder="$t('invoice.Shippingtotal')" @keyup.enter.native="changeShippingAmount" @change="changeShippingAmount">
                              </el-input>
                            </div>
                          </div>
                          <el-divider></el-divider>
                          ...
                          <div class="d-flex justify-content-end mg-t-10" v-show="discount>0">
                            <div class="">{{$t('invoice.Discount')}} (US$) :</div>
                          ...
                          <div class="d-flex justify-content-end mg-t-10 bold-input">
                            <div class="tx-bold">{{$t('invoice.Totalamount')}}</div>
                            <div class="mg-l-5">
                              <template v-if="invoiceCnt.notBindCnt > 0">
                                <el-tooltip class="item" effect="dark" placement="bottom">
                                  ...
                                  <div class="">
                                    <el-input :disabled="invoiceInfo.payStatus == 1 || invoiceCnt.notBindCnt > 0" name="price-input" size="small" v-model="realAmount" :placeholder="$t('invoice.Totalamount')" @keyup.enter.native="changeRealAmount" @change="changeRealAmount">
                                    </el-input>
                                  </div>
                                </el-tooltip>
                              </template>
                              <template v-else>
                                <el-input :disabled="isChangeAmount()" name="price-input" size="small" v-model="realAmount" :placeholder="$t('invoice.Totalamount')" @keyup.enter.native="changeRealAmount" @change="changeRealAmount">
                                </el-input>
                              </template>
                            </div>
                          </div>
                          <div class="d-flex justify-content-end mg-t-10 set-paytype-3" style="align-items: flex-start;">
                            <div class="">{{$t('invoice.Paymentmethod')}} :</div>
                            <template v-if="invoiceCnt.notBindCnt > 0">
                              <el-tooltip class="item" effect="dark" placement="bottom" popper-class="tip-with-hover">
                                <div slot="content">
                                  {{$t('invoice.stillInvoice')}}
                                  <el-button @click="changeTabByTip" type="text" size="mini"><i style="color: #F56C6C;" class="fa fa-exclamation-triangle"></i></el-button>
                                  {{$t('invoice.stillmissing')}}<br />{{$t('invoice.invoicedetails')}}
                                ...
                                <div class="mg-l-5">
                                  <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1 || invoiceInfo.status == '9'" style="display: block; text-align: left;margin: 5px 0 10px;" v-model="paymentType" label="1">{{$t('invoice.PayPal')}}</el-radio>
                                  <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1 || invoiceInfo.status == '9'" style="display: block; text-align: left;" v-model="paymentType" label="2">{{$t('invoice.AliExpress')}}</el-radio>
                                </div>
                              </el-tooltip>
                        ...
                        <div v-if="invoiceInfo.status != '9'" class="tx-right mg-t-15 d-flex justify-content-end">
                          <template v-if="invoiceCnt.notBindCnt > 0">
                            <el-tooltip class="item" effect="dark" placement="bottom">
                              <div slot="content">
                                {{$t('invoice.stillInvoice')}}
                              ...
                              <el-button :disabled="invoiceInfo.status == '9'" class="with-disabled step-btn-4" size="medium" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                            </el-tooltip>
                          </template>
                          <template v-else-if="invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2">
                            <el-tooltip class="item" effect="dark" placement="bottom">
                              <div slot="content">{{$t('invoice.paymentmethod')}}</div>
                              <el-button :disabled="invoiceInfo.status == '9'" class="with-disabled step-btn-4" size="medium" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                            </el-tooltip>
                          </template>
                          <template v-else-if="invoiceInfo.paymentType == 1 || invoiceInfo.paymentType == 2">
                            <el-button :disabled="invoiceInfo.status == '9'" @click="sendInvoice()" class=" step-btn-4" size="medium" type="success" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                          </template>

                          <template v-if="invoiceCnt.notBindCnt == 0">

                            <div class="d-flex btn-wrap">
                              <template v-if="invoiceInfo.payStatus == 2 && (invoiceInfo.paymentType == 1 || invoiceInfo.paymentType == 2)">
                                <el-tooltip :disabled="!hasAbnormal" effect="dark" content="采购数量异常，请处理后再操作" placement="top-start">
                                  <el-popover
                                    placement="top"
                                    width="460"
                                  ...
                                  >
                                    <div>
                                      <div style="line-height: 20px;" class="mg-b-5">{{$t('invoice.Received')}}</div>
                                      <div style="line-height: 20px;" class="tx-error mg-b-10">{{$t('invoice.Afterconfirmation')}}</div>
                                    </div>
                                    <div style="text-align: right; margin: 0">
                                      <el-button size="mini" type="text" @click="confirmVisible = false">{{$t('invoice.cancel')}}</el-button>
                                      <el-button type="primary" size="mini" @click="confirmMoneyOnly('1')">{{$t('invoice.confirm')}}</el-button>
                                    </div>
                                    <a @click="showPopover" slot="reference" class="btn-item btn-item1" :class="{' with-disabled': invoiceInfo.status == '9' || hasAbnormal}" href="javascript:;">
                                      <i class="fa fa-dollar"></i>
                                      <span>{{$t('invoice.Confirmation')}}</span>
                                    </a>
                                  </el-popover>
                                </el-tooltip>
                              </template>
                              <template v-if="invoiceInfo.status == '9' || (invoiceInfo.payStatus == 2 && (invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2))">
                                <el-tooltip class="item" effect="dark" placement="bottom">
                                  <div slot="content">{{$t('invoice.paymentmethod')}}</div>
                                  <a class="btn-item btn-item1 with-disabled" href="javascript:;">
                                    <i class="fa fa-dollar"></i>
                                    <span>{{$t('invoice.Confirmation')}}</span>
                                  </a>
                                </el-tooltip>
                              ...
                              <template v-if="invoiceInfo.payStatus == 1">
                                <el-tooltip class="item" effect="dark" placement="bottom">
                                  <div slot="content">{{$t('invoice.haveconfirmed')}}</div>
                                  <a class="btn-item btn-item1 with-disabled" href="javascript:;">
                                    <i class="fa fa-dollar"></i>
                                    <span>{{$t('invoice.Confirmation')}}</span>
                                  </a>
                                ...
                                <el-tooltip class="item" effect="dark" placement="bottom">
                                  <div slot="content">{{$t('invoice.Clickbutton')}}</div>
                                  <a class="btn-item btn-item2 confirmStep1" href="javascript:;" @click="confirmMoney">
                              ...
                              <template v-else>
                                <el-tooltip class="item" effect="dark" placement="top">
                                  ...
                                  <a
                                    class="btn-item btn-item2 with-disabled confirmStep1"
                                    :class="(invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2) || hasAbnormal ? 'with-disabled-btn1' : ''"
                                    href="javascript:;">
                                    <i class="el-icon-connection"></i>
                                    <span>{{$t('invoice.ERPsystem')}}</span>
                                  </a>
                  ...
                  <div v-show="!noMore" class="page-footer-fixed">
                    <div class="tour-mask" v-show="invoiceConfirmTourShow"></div>
                      ...
                      <div class="without-hover">
                        <el-row style="margin: 20px 15px 10px -15px;">
                          ...
                          <el-col :span="24">
                            <div class="d-flex justify-content-between">
                              <div class="d-flex">
                                <div v-show="invoiceInfo.customerRead == 1" style="display: flex;align-items: center;color: #67C23A;margin-right: 20px;">
                                  <i class="el-icon-success" style="font-size: 16px;"></i>
                                  <div style="margin-left: 5px;">
                                    {{invoiceInfo.timeRead ? invoiceInfo.timeRead : "---"}}
                                    <span style="display: block;white-space: normal;">{{$t('invoice.Customerconfirmed')}}</span>
                                  </div>
                                </div>
                                <div v-show="invoiceInfo.payStatus == 1" style="display: flex;align-items: center;color: #67C23A;margin-right: 20px;">
                                  <i class="el-icon-success" style="font-size: 16px;"></i>
                                  <div style="margin-left: 5px;">
                                    {{invoiceInfo.timePaid ? invoiceInfo.timePaid : "---"}}
                                    <span style="display: block;white-space: normal;">{{$t('invoice.Supplierconfirmed')}}</span>
                                  </div>
                                </div>
                                <div v-show="invoiceInfo.timeApiGet" style="display: flex;align-items: center;color: #67C23A;margin-right: 20px;">
                                  <i class="el-icon-success" style="font-size: 16px;"></i>
                                  <div style="margin-left: 5px;">
                                    {{invoiceInfo.timeApiGet ? invoiceInfo.timeApiGet : "---"}}
                                    <span style="display: block;white-space: normal;">{{$t('invoice.ERPdownloaded')}}</span>
                                  </div>
                                </div>
                                <div v-show="invoiceInfo.firstDeliverTime" style="display: flex;align-items: center;color: #67C23A;margin-right: 20px;">
                                  <i class="el-icon-success" style="font-size: 16px;"></i>
                                  <div style="margin-left: 5px;">
                                    {{invoiceInfo.firstDeliverTime ? invoiceInfo.firstDeliverTime : "---"}}
                                    <span style="display: block;white-space: normal;">{{$t('invoice.Supplierstarts')}}</span>
                                  </div>
                                </div>
                              </div>
                              <div class="d-flex justify-content-end" style="white-space: nowrap;">
                                ...
                                <div class="mg-l-5" style="font-weight: bold;font-size: 16px!important;">
                                  {{realAmount > 0 ? $numberToCurrency(realAmount) : $t('invoice.Notset')}}
                                </div>
                              </div>
                            </div>
                          </el-col>
                        ...
                        <el-row style="margin: 10px 15px 10px -15px;">
                          ...
                          <el-col :span="16" style="overflow: visible;">
                            <div class="grid-content invoice-bottom">
                              ...
                              <el-divider style="margin: 20px -20px 20px 20px;"></el-divider>
                              <div class="d-flex justify-content-end">
                                <div class="">{{$t('invoice.AmountDue')}} (US$) :</div>
                                <div class="mg-l-5 pd-r-15">{{realAmount > 0 ? $numberToCurrency(totalAmount) : $t('invoice.Notset')}}</div>
                              </div>
                                ...
                                <div class="mg-l-5">
                                  <template v-if="invoiceCnt.notBindCnt > 0">
                                    ...
                                    <div class="tooltip-wrap">
                                      <div class="">
                                        <el-input style="font-weight: bold; font-size: 16px!important;" :disabled="invoiceInfo.payStatus == 1 || invoiceCnt.notBindCnt > 0" name="price-input" size="small" v-model="realAmount" :placeholder="$t('invoice.Totalamount')" @keyup.enter.native="changeRealAmount" @change="changeRealAmount">
                                        </el-input>
                                      </div>
                                  ...
                                  <template v-else>
                                    <el-input style="font-weight: bold; font-size: 16px!important;" :disabled="isChangeAmount()" name="price-input" size="small" v-model="realAmount" :placeholder="$t('invoice.Totalamount')" @keyup.enter.native="changeRealAmount" @change="changeRealAmount">
                                    </el-input>
                                  </template>
                                </div>
                              </div>
                              <div class="d-flex justify-content-end mg-t-10" style="align-items: flex-start;">
                                ...
                                <template v-if="invoiceCnt.notBindCnt > 0">
                                  ...
                                  <div class="tooltip-wrap">
                                    <div class="mg-l-5">
                                      <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1" style="display: block; text-align: left;margin: 5px 0 10px;" v-model="paymentType" label="1">{{$t('invoice.PayPal')}}</el-radio>
                                      <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1" style="display: block; text-align: left;" v-model="paymentType" label="2">{{$t('invoice.AliExpress')}}</el-radio>
                                    </div>
                                    <span class="tooltiptext">
                                  {{$t('invoice.stillInvoice')}}
															<el-button @click="changeTabByTip" type="text" size="mini"><i style="color: #F56C6C;" class="fa fa-exclamation-triangle"></i></el-button>
                                ...
                                <template v-else>
                                  <div class="mg-l-5">
                                    <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1" style="display: block; text-align: left;margin: 5px 0 10px;" v-model="paymentType" label="1">{{$t('invoice.PayPal')}}</el-radio>
                                    <el-radio @change="setPayment" :disabled="invoiceCnt.notBindCnt > 0 || invoiceInfo.payStatus == 1" style="display: block; text-align: left;" v-model="paymentType" label="2">{{$t('invoice.AliExpress')}}付款</el-radio>
                                  </div>
                                </template>
                              </div>
                              <div v-if="paymentType == 2" class="mg-t-10 normal-input">
                            ...
                            <div v-if="invoiceInfo.status != '9'" class="tx-right mg-t-15 d-flex justify-content-end">
                              <template v-if="invoiceCnt.notBindCnt > 0">
                                ...
                                <div class="tooltip-wrap">
                                  <el-button :disabled="invoiceInfo.status == '9'" class="with-disabled" size="medium" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                                  <span class="tooltiptext">
												        	{{$t('invoice.stillInvoice')}}
													<el-button @click="changeTabByTip" type="text" size="mini"><i style="color: #F56C6C;" class="fa fa-exclamation-triangle"></i></el-button>
													{{$t('invoice.stillmissing')}}<br />{{$t('invoice.invoicedetails')}}
                              ...
                              <template v-else-if="invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2">
                                ...
                                <div class="tooltip-wrap">
                                  <el-button :disabled="invoiceInfo.status == '9'" class="with-disabled" size="medium" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                                  <span class="tooltiptext">{{$t('invoice.paymentmethod')}}</span>
                                </div>
                              </template>
                              <template v-else-if="invoiceInfo.paymentType == 1 || invoiceInfo.paymentType == 2">
                                <el-button :disabled="invoiceInfo.status == '9'" @click="sendInvoice()" size="medium" type="success" icon="fa fa-send" style="margin-right: 10px;">{{$t('invoice.Sendinvoiceto')}}</el-button>
                              </template>
                              ...
                              <template v-if="invoiceCnt.notBindCnt == 0">
                                <div class="d-flex btn-wrap">
                                  <template v-if="invoiceInfo.payStatus == 2 && (invoiceInfo.paymentType == 1 || invoiceInfo.paymentType == 2)">
                                    <el-tooltip :disabled="!hasAbnormal" effect="dark" content="采购数量异常，请处理后再操作" placement="top-start">
                                      ...
                                      >
                                        ...
                                        <div style="text-align: right; margin: 0">
                                          <el-button size="mini" type="text" @click="confirmVisible2 = false">{{$t('invoice.cancel')}}</el-button>
                                          <el-button type="primary" size="mini" @click="confirmMoneyOnly('2')">{{$t('invoice.confirm')}}</el-button>
                                        </div>
                                        <a @click="showPopover2" slot="reference" class="btn-item btn-item1" :class="{' with-disabled': invoiceInfo.status == '9' || hasAbnormal}" href="javascript:;">
                                          <i class="fa fa-dollar"></i>
                                          <span>{{$t('invoice.Confirmation')}}</span>
                                        </a>
                                      </el-popover>
                                  ...
                                  <template v-if="invoiceInfo.payStatus == 1">
                                    ...
                                    <el-tooltip class="item" effect="dark" placement="bottom">
                                      <div slot="content">{{$t('invoice.Clickbutton')}}</div>
                                      <a class="btn-item btn-item2 confirmStep" href="javascript:;" @click="confirmMoney">
                                        <i class="el-icon-connection"></i>
                                        <span>{{$t('invoice.ERPsystem')}}</span>
                                      </a>
                                    </el-tooltip>
                                  ...
                                  <template v-else>
                                    <div class="tooltip-wrap">
                                      ...
                                      <span class="tooltiptext">{{$t('invoice.receivepayment')}}</span>
                                    </div>
                                  </template>
                                </div>
                              </template>
                              <template v-if="false">
                                <template v-if="invoiceInfo.payStatus == 2 && (invoiceInfo.paymentType == 1 || invoiceInfo.paymentType == 2)">
                                  <el-popover @hide="hidePopover" placement="top" width="460" v-model="confirmVisible2">
                                    <div>
                                      <div style="line-height: 20px;" class="mg-b-5">{{$t('invoice.Received')}}</div>
                                      <div style="line-height: 20px;" class="tx-error mg-b-10">{{$t('invoice.Afterconfirmation')}}</div>
                                    ...
                                    <el-button @click="showPopover2" slot="reference" size="medium" type="primary" icon="fa fa-dollar">{{$t('invoice.Confirmation')}}</el-button>
                                  </el-popover>
                                </template>
                                <template v-if="invoiceInfo.payStatus == 2 && (invoiceInfo.paymentType != 1 && invoiceInfo.paymentType != 2)">
                                  <div class="tooltip-wrap">
                                    <el-button class="with-disabled" size="medium" type="primary" icon="fa fa-dollar">{{$t('invoice.Confirmation')}}</el-button>
                                    <span class="tooltiptext">{{$t('invoice.paymentmethod')}}</span>
                                  </div>
                                </template>

                                <template v-if="invoiceInfo.payStatus == 1">
                                  <el-button class="confirmStep" style="margin-left: 10px;" @click="confirmMoney" size="medium" type="primary" icon="el-icon-connection">{{$t('invoice.ERPsystem')}}</el-button>
                                </template>
                                <template v-else>
                                  <div class="tooltip-wrap">
                                    <el-button style="margin-left: 10px;" class="with-disabled" size="medium" type="primary" icon="el-icon-connection">{{$t('invoice.ERPsystem')}}</el-button>
                                    <span class="tooltiptext">{{$t('invoice.receivepayment')}}</span>
                                  </div>
                                </template>
                              </template>
              ...
              <template v-if="tabActive == 5">
                <div class="table-wrap">
                  ...
                  <el-table :row-class-name="tableRowClassName" :header-cell-style="getRowClass" :cell-style="cellStyle" :data="invoiceData2" stripe border class="table-with-body table-with-body2">
                    <el-table-column width="150" prop="sku" :label="$t('invoice.SupplierSKU')">
                    </el-table-column>
                    <el-table-column :label="$t('invoice.Orderitems')">
                    </el-table-column>
                    <el-table-column prop="qty" :label="$t('invoice.Purchasequantity')" width="160">
                    </el-table-column>
                    <el-table-column prop="price" :label="$t('invoice.Supplyunit')" width="120">
                    </el-table-column>
                    <el-table-column :label="$t('invoice.Totalsupply')" width="120">
                    ...
                    <template slot="empty">
                      <div @click="tabcChange(1)" style="text-align: center;cursor: pointer;padding-top: 20px;">
                        <span>{{$t('invoice.Expandproduct')}}</span>
            ...
            <div v-show="showDownloadInvoice" class="download-invoice" style="position: relative;left: 9999px;">
              <el-card shadow="always">
                ...
                <template>
                  <div class="table-wrap">
                    ...
                    <el-table :row-class-name="tableRowClassName" :header-cell-style="getRowClass" :cell-style="cellStyle" :data="invoiceData3" stripe border class="table-with-body">
                      <el-table-column label="Items">
                        <template slot-scope="scope">
                          <div class="">
                            <div style="max-width: 100%; text-align: left;">
                              ...
                              <div class="d-flex no-wrap justify-content-between" v-if="tabActive != 2">
                                <el-popover popper-class="customer-popover" placement="bottom" :title="`Shopify Order ${scope.row.codeName}`" width="500" trigger="hover">
                                  ...
                                  <div slot="reference" style="cursor: pointer;">
                                    <span style="margin-right: 10px;">{{scope.row.codeName}}<i class="el-icon-caret-bottom el-icon--right"></i></span>
                                    <span style="display: inline-block;max-width: 100px;text-overflow: ellipsis;overflow: hidden;" v-if="scope.row.shippingJson && scope.row.shippingJson.name">To {{scope.row.shippingJson.name}} </span>
                                    <span style="display: inline-block;max-width: 100px;text-overflow: ellipsis;overflow: hidden;" v-if="scope.row.shippingJson && scope.row.shippingJson.country"> ({{scope.row.shippingJson.country}})</span>
                                  </div>
                                  <div class="mg-y-15 mg-t-20">
                                    <div class="d-flex">
                                      <div class="d-flex" style="width: 90%;">
                      ...
                      <el-table-column prop="qty" label="Quantity" width="160">
                        <template slot-scope="scope">
                          ...
                          <div v-if="tabActive == 2">{{scope.row.quantity}}</div>
                          <el-tooltip class="item" effect="dark" :content="$t('invoice.Abnormalreasons') + scope.row.abnormalReason" placement="right">
                            <span v-if="scope.row.isAbnormal == 1" class="tx-danger" style="margin-top: 10px;display: inline-block;"><i class="el-icon-info"></i>{{$t('invoice.Abnormal')}}</span>
                          </el-tooltip>
                        </template>
                      </el-table-column>
                    ...
                    <el-row>
                      <el-col :span="20" :offset="4">
                        <div class="grid-content invoice-bottom">
                          ...
                          <div class="d-flex justify-content-end mg-t-10" v-if="discount && discount>0">
                  ...
                  <div class="d-flex" style="align-items: flex-end;">
                    ...
                    <div>
                      ...
                      <el-link :underline="false" style="text-decoration: underline;margin: 0 3px;">{{invoiceInfo.url}}</el-link>
                    </div>
                  </div>
                </template>
              </el-card>
          ...
          <el-aside width="50px" style="position: relative;left: 50px;">
            <template>
              <el-tooltip v-if="invoiceInfo.status != '9'" class="item" effect="dark" :content="$t('invoice.exportImport')" placement="right">
                <el-button size="medium" circle @click="opendrawer('3')" type="primary" style="position: fixed;bottom: 260px;width: 40px;height: 40px;line-height: 40px;text-align: center;padding: 0;z-index: 1;">
                  <i class="iconfont icon-xiazai" style="margin-right: 0;font-size: 22px;"></i>
                </el-button>
              </el-tooltip>
            </template>

            <el-tooltip class="item" effect="dark" :content="$t('invoice.Orderstatistics')" placement="right">
            ...
            <el-tooltip class="item" effect="dark" :content="$t('invoice.Helpcenter')" placement="right">
              <el-button size="medium" circle type="primary" @click="opendrawer('2')" style="margin-left:0;position: fixed;bottom: 140px;width: 40px;height: 40px;line-height: 40px;text-align: center;padding: 0;z-index: 1;">
      ...
      <el-container v-show="(tabActive != 5 && noMore) || tabActive == 5" id="messageContainer">
        <el-main>
          <div class="d-flex justify-content-between mg-b-20 pd-x-20">
            ...
            <el-checkbox v-model="checked">{{$t('invoice.Showmessage')}}</el-checkbox>
          </div>
          <el-divider></el-divider>
          <el-row :gutter="30">
            <el-col :span="12">
            ...
            <el-col :span="12">
              <div v-if="invoiceInfo.status != '9'" class="mg-b-15" style="margin-left: 40px;position: relative;background: #fff;border: 1px solid #ddd;padding: 10px;">
                ...
                <!--v-model="msgContent" -->
                <textarea :placeholder="$t('invoice.Pleasemessage')" class="form-control textarea-control" autosize resize="none" data-emojiable="true"></textarea>

                <div style="display: flex;align-items: center;justify-content: flex-end;position: absolute;bottom: 10px;right: 10px;">

                  <el-upload class="upload-demo" action="" accept="image/jpeg,image/jpg,image/png" :show-file-list="false" :http-request="uploadSectionFile2">
                    <i class="fa fa-paperclip" style="font-size: 20px;margin: 0 10px;"></i>
                  </el-upload>
                  <el-button slot="append" @click="addMessage" size="small">{{$t('ticket.send')}}</el-button>
                </div>
                        ...
                        <el-card>
                          <div class="d-flex timeline-customer">
                            ...
                            <div class="mg-l-10">
                              ...
                              <p class=" mg-t-5" style="word-break: break-all;" v-html="activity.content"></p>
      ...
      <el-dialog custom-class="addCustomer" :title="$t('invoice.newcustomer')" :visible.sync="addCustomerOpen" width="40%">
        <el-divider></el-divider>
        <el-form :rules="formNewcustomerRules" ref="formNewcustomer" :model="formNewcustomer" :label-position="'right'" label-width="100px">
          <el-form-item :label="$t('mycustomer.customersName')" prop="name">
            <el-input v-model="formNewcustomer.name" :placeholder="$t('invoice.Pleasename')"></el-input>
          </el-form-item>
          <el-form-item :label="$t('importdata.cusEmail')" prop="email">
            <el-input v-model="formNewcustomer.email" :placeholder="$t('invoice.PleaseEmail')"></el-input>
          </el-form-item>
          <el-form-item :label="$t('mycustomer.remarks')" prop="remark">
            <el-input v-model="formNewcustomer.remark" :placeholder="$t('invoice.Pleaseremarks')"></el-input>
          </el-form-item>
        </el-form>
        <el-divider></el-divider>
        <div slot="footer" class="dialog-footer">
          <el-button @click="addCustomerOpen = false">{{$t('invoice.cancel')}}</el-button>
          <el-button type="primary" @click="submitAddcustomer('formNewcustomer')">{{$t('invoice.confirm')}}</el-button>
        </div>
      </el-dialog>
      <el-dialog custom-class="editCustomer" :title="$t('invoice.modifiedCustomer')" :visible.sync="editCustomerOpen" width="40%">
        <el-divider></el-divider>
        <el-form :rules="formEditcustomerRules" :model="formEditcustomer" ref="formEditcustomer" :label-position="'right'" label-width="100px">
          <el-form-item :label="$t('mycustomer.customersName')" prop="name">
            <el-input v-model="formEditcustomer.name" :placeholder="$t('invoice.Pleasename')"></el-input>
          </el-form-item>
          <el-form-item :label="$t('importdata.cusEmail')" prop="email">
            <el-input v-model="formEditcustomer.email" :placeholder="$t('invoice.PleaseEmail')"></el-input>
          </el-form-item>
          <el-form-item :label="$t('mycustomer.remarks')" prop="remark">
            <el-input v-model="formEditcustomer.remark" :placeholder="$t('invoice.Pleaseremarks')"></el-input>
          </el-form-item>
        </el-form>
        <el-divider></el-divider>
        <div slot="footer" class="dialog-footer">
          <el-button size="small" @click="editCustomerOpen = false">{{$t('invoice.cancel')}}</el-button>
          <el-button size="small" type="primary" @click="submitEditcustomer('formEditcustomer')">{{$t('invoice.confirm')}}</el-button>
        </div>
      </el-dialog>
      <el-dialog custom-class="editCompany" :title="$t('invoice.modifiedCompany')" :visible.sync="editCompanyInfoOpen" width="40%">
        <el-divider></el-divider>
        <el-form :rules="formEditCompanyRules" ref="formEditCompany" :model="formEditCompany" :label-position="'right'" label-width="150px">
          <el-form-item :label="$t('invoice.companyName')" prop="name">
            <el-input v-model="formEditCompany.name" :placeholder="$t('invoice.fillcompany')"></el-input>
          </el-form-item>
        </el-form>
        <el-divider></el-divider>
        <div slot="footer" class="dialog-footer">
          <el-button size="small" @click="editCompanyInfoOpen = false">{{$t('invoice.cancel')}}</el-button>
          <el-button size="small" type="primary" @click="submitEditCompany('formEditCompany')">{{$t('invoice.confirm')}}</el-button>
        </div>
      </el-dialog>
      <el-dialog :title="$t('invoice.shippedMark')" :visible.sync="dialogMarkShipped.isShow" width="700px">
          ...
          <el-col :span="24">
            <el-form label-width="160px">
              ...
              <el-form-item :label="$t('invoice.Logisticsnumber')" required>
                <el-input v-model="dialogMarkShipped.trackNumber" :placeholder="$t('invoice.Logisticsnumber')"></el-input>
              </el-form-item>
              <el-form-item :label="$t('invoice.Logisticscompany')" required>
                <!--:style="{width:(dialogMarkShipped.trackKey == 99?'50%':'100%')}"-->
                <el-select style="width: 100%;" v-model="dialogMarkShipped.trackKey" :placeholder="$t('invoice.Logisticscompany')">
                  <el-option v-for="item in dialogMarkShipped.shopifyCompany" :key="item.key" :label="item.name" :value="item.key">
                  </el-option>
                </el-select>
                <el-input style="margin: 10px 0;" v-if="dialogMarkShipped.trackKey == 99" v-model="dialogMarkShipped.trackCompany" :placeholder="$t('invoice.Logisticscompany')"></el-input>
              </el-form-item>
              <el-form-item v-if="dialogMarkShipped.trackKey == 99" :label="$t('invoice.Trackinglinks')" required>
                <el-input v-model="dialogMarkShipped.trackUrl" :placeholder="$t('invoice.Trackinglinks')"></el-input>
              </el-form-item>
        ...
        <div slot="footer" class="dialog-footer d-flex justify-content-between">
          ...
          <el-button type="primary" :loading="dialogMarkShipped.loading" @click="saveMarkShipped">{{$t('invoice.shippedMark')}}</el-button>
        </div>
      </el-dialog>
      <el-dialog @open="openSkype" custom-class="sendInvoice" :title="$t('invoice.Sendinvoiceto')" :visible.sync="sendInvoiceOpen" width="50%">
        <!--<el-divider></el-divider>-->
        <template>
    ...
    top: 15px;">
          ...
          <el-tabs v-model="activeShareName" type="card" style="margin-left: 20px;margin-top: 10px;">
            ...
            <el-tab-pane name="shareWeixin">
              <span slot="label"><i class="iconfont icon-shejiao"></i> {{$t('invoice.Sharethrough')}}</span>
              <div>
                <div class="d-flex mg-y-20">
                  <div style="color: #999;width: 150px;text-align: right;margin-right: 10px;white-space: nowrap;">{{$t('invoice.SetshareTitle')}}</div>
                  <div style="width: 70%;">
                    <el-input @change="saveRemark('2')" type="text" size="small" v-model="shareTitle"></el-input>
                  </div>
                </div>
                <div class="d-flex mg-y-20 align-items-center">
                  <div style="color: #999;width: 150px;text-align: right;margin-right: 10px;white-space: nowrap;">{{$t('invoice.Setsharecontent')}}</div>
                  <div style="width: 70%;">
                ...
                <div class="d-flex mg-y-20">
                  <div style="color: #999;width: 150px;text-align: right;margin-right: 10px;white-space: nowrap;">{{$t('invoice.Sharemethod')}}</div>
                  <div style="width: 70%;" class="d-flex">
                    ...
                    <a class="share-weixin share-download" href="javascript:;" @click="downloadInvoice">
                      <i class="fa fa-download"></i> {{$t('invoice.Downloadinvoice')}}
                    </a>
                  </div>
                </div>
              </div>
        ...
        <div slot="footer" class="dialog-footer d-flex justify-content-between">
          ...
          <div>
            <el-button size="small" @click="sendInvoiceOpen = false">{{$t('invoice.cancel')}}</el-button>
      ...
      <el-dialog custom-class="confirmMoney" :title="$t('invoice.processorders')" :visible.sync="confirmMoneyOpen" width="40%">
        <el-dialog :title="$t('ticket.submitTicket')" :visible.sync="DialogEditComment.visible" width="760px" @closed="$refs.uploadFile.clearFiles()" append-to-body>
          <el-divider></el-divider>
          <div>
            <el-form ref="CommentForm">
              ...
              <el-form-item label="">
                <div style="font-size: 16px; font-weight: bold;margin: 20px 0 0;">{{$t('ticket.uploadImageAttachment')}}</div>
              </el-form-item>
              <el-form-item label="">
                <el-upload ref="uploadFile" :on-change="handleChangeImg" :before-remove="handleRemoveImg" action="" :auto-upload="false" accept="image/gif,image/jpeg,image/jpg,image/png" class="upload-file">
                  <el-button ref="uploadBtn" size="small" type="primary" :loading="DialogEditComment.uploadLoading">{{$t('ticket.browse')}}</el-button>
                </el-upload>
              </el-form-item>
          ...
          <div slot="footer" class="dialog-footer d-flex justify-content-end">
            <div class=" mg-r-15">
              <el-button size="small" type="primary" :loading="DialogEditComment.loading" @click="saveComment">{{$t('ticket.submitTicket')}}</el-button>
            </div>
            <div>
              <el-button size="small" @click="DialogEditComment.visible = false">{{$t('invoice.cancel')}}</el-button>
            </div>
          </div>
        </el-dialog>

        <el-dialog :title="$t('invoice.usertoken')" :visible.sync="secretKeyOpen" width="50%" :close-on-click-modal="false" append-to-body>
          <el-divider></el-divider>
          ...
          <div style="margin: 0 20px;padding: 15px;background-color: #fafafa;border: 1px solid #eaeefb;border-radius: 4px;color: #303133;">
            <p>
            ...
            <p style="margin-top: 10px;">
              {{$t('invoice.present')}}
              <el-link target="_blank" type="primary" :underline="false" href="//www.mabangerp.com/">马帮ERP</el-link>
              {{$t('invoice.soon')}}
            </p>
          </div>
          <div style="margin: 20px;color: rgb(153, 153, 153);">
            <p>
              {{$t('invoice.sametime')}}
              <el-link type="primary" :underline="false" @click="openAddComment">{{$t('invoice.contact')}}</el-link>。
            </p>
          </div>
          <el-divider></el-divider>
          <div slot="footer" class="dialog-footer">
            <el-button size="small" type="primary" @click="secretKeyOpen = false">{{$t('importdata.gotIt')}}</el-button>
          </div>
        </el-dialog>
        <el-tabs v-model="activeConfirm" type="card" style="margin-left: 20px;margin-top: 10px;">
          <el-tab-pane name="confirmErp">
            <span slot="label"> {{$t('invoice.authorizederp')}}</span>
            <div>
              <div class="mg-y-20">
                <span style="margin: 0 10px 0 26px;display: inline-block;width: 177px;text-align: right;">{{$t('invoice.tokenerp')}}</span>
                <span>
									{{userInfo && userInfo.secretKey ? userInfo.secretKey : "---"}}
								</span>
                <el-button v-if="userInfo && userInfo.secretKey" size="mini" type="text">
                  <i class="el-icon-document-copy" v-clipboard:copy="userInfo.secretKey" v-clipboard:success="onCopy" v-clipboard:error="onError"></i>
                </el-button>
              ...
              <div style="display: flex;align-items: center;" class="mg-y-20">
                ...
                <el-autocomplete :disabled="!!invoiceInfo.timeApiGet" size="medium" class="inline-input" v-model="shopName" :fetch-suggestions="querySearch" :placeholder="$t('invoice.entershopname')" @select="handleSelect"></el-autocomplete>
              </div>
              <div class="mg-y-20" v-if="invoiceInfo.timeApiGet">
                <span style="margin: 0 10px 0 26px;opacity: 0;">{{$t('invoice.storename')}}</span>
                <span class="tx-error">
									<span style="margin: 0 5px;">{{$t('invoice.ERPdownloaded')}} </span>{{invoiceInfo.timeApiGet ? invoiceInfo.timeApiGet : "---"}}
								</span>
              </div>
              <div class="mg-y-20">
                <span style="margin: 0 10px 0 26px;opacity: 0;">{{$t('invoice.storename')}}</span>
                <el-button :disabled="!!invoiceInfo.timeApiGet" size="small" icon="fa fa-save" type="primary" @click="confirmMoneyFn()">{{$t('invoice.savesettings')}}</el-button>
              </div>
            </div>
          </el-tab-pane>
          <el-tab-pane name="confirmExcel">
            ...
            <div class="mg-y-20">
              <p style="margin-left: 50px;" v-html="$t('invoice.qualifiedorders',{num: Number(invoiceCnt.orderCnt)})"></p>
            </div>
            <div style="margin-left: 50px;" class="mg-y-20">
              <el-button size="small" icon="iconfont icon-excel3" type="success" @click="saveCustomExportOnly('1')">{{$t('invoice.Exportexcel')}}</el-button>
              <el-button size="small" icon="iconfont icon-csv11" type="success" @click="saveCustomExportOnly('2')">{{$t('invoice.Exportcsv')}}</el-button>
              <el-button size="small" icon="fa fa-cog" plain type="primary" @click="setExportOnly()">{{$t('invoice.exportfields2')}}</el-button>
            </div>
            <el-dialog width="80%" :title="$t('invoice.exportfields2')" :visible.sync="innerVisible" append-to-body>
              <div class="modal-export">
                <el-divider></el-divider>
                <div class="border-bottom-1 mg-b-15 pd-b-15">
                  <h6 class="mg-b-15" style="border-left: 4px solid #3F4EAE;padding-left: 10px;">{{$t('invoice.exportFields')}}<span class="mg-l-5">({{$t('invoice.dragging')}})</span></h6>
                  <div class=" draggable-wrap">
                    ...
                    <div v-if="!modalCustomExport.selectedTitles.length" class=" w-100 justify-content-center">
                      <span style="color: #F56C6C;">{{$t('invoice.selectPlease')}}</span>
                    </div>
                  </div>
                </div>
                <el-divider></el-divider>
                ...
                <div class="border-bottom-1 mg-b-15 pd-b-15">
                  <h6 class="mg-b-15" style="border-left: 4px solid #3F4EAE;padding-left: 10px;">
                    ...
                    <a style="color: #5C6AC4;text-decoration: none;" class="mg-l-20" href="javascript:;" @click="customWrapState()">{{showCustom ? $t('invoice.fieldsHide') : $t('invoice.fieldsShow')}}</a>
                  </h6>
              ...
              <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="innerVisible = false">{{$t('invoice.cancel')}}</el-button>
        ...
        <el-divider></el-divider>
        <div slot="footer" class="dialog-footer">
          <el-button size="small" @click="confirmMoneyOpen = false">{{$t('invoice.cancel')}}</el-button>
      ...
      <el-dialog
        ...
        :element-loading-text="setLotPriceDialog.step == 4 ? '批量改价已提交请稍后 ':''"
      ...
      >
        ...
        <template v-if="setLotPriceDialog.step == 1">
          <el-row>
            ...
            <el-col :offset="2" :span="7">
              <div class="tx-center set-price other" @click="setLotPriceStep(2)">
                <div class="logo-wrap">
                  其他
                </div>
                <p>其他ERP授权</p>
        ...
        <template v-if="setLotPriceDialog.step == 2">
          <el-form ref="form" :model="setLotPriceDialog" label-width="160px" size="medium">
            <el-form-item label="当前授权ERP：">
              <span v-if="userInfo.source == 2">马帮ERP2.0</span>
              <span v-if="userInfo.source == 3">马帮ERP3.0</span>
              <a v-if="userInfo.source == 3" href="javascript:;" class="mg-l-20" @click="unbind()">解除绑定</a>
            </el-form-item>
            <el-form-item class="mg-t-15" label="供货价和运费：">
              <el-radio-group v-model="setLotPriceDialog.price" @change="changePriceType">
                <el-radio label="1">供货单价=（商品成本价+物流成本）+单价利润，无总运费</el-radio>
                <el-radio v-if="userInfo.source == 3" class="mg-t-10" label="2">供货单价=商品成本价+商品目标利润，总运费=物流成本+物流目标利润</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item v-if="userInfo.source == 3" class="mg-t-15" label="运费规则：">
              <el-select style="width: 300px;" filterable v-model="setLotPriceDialog.ruleId" placeholder="请选择运费规则">
                <el-option label="请选择运费规则" value=""></el-option>
                <el-option v-for="rule in setLotPriceDialog.erpRules" :label="rule.name" :value="rule.shippingFeeRuleId" :key="rule.shippingFeeRuleId"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="mg-t-15" label="设置商品：">
              <el-radio-group v-model="setLotPriceDialog.onlyNone" v-if="!setLotPriceDialog.sku">
                <el-radio label="1" :disabled="setLotPriceDialog.price == 2">仅设置 [未填写供货价] 商品</el-radio>
                <el-radio class="mg-t-10 d-block" label="">当前采购订单 [全部] 商品</el-radio>
              </el-radio-group>
              <div v-else>{{setLotPriceDialog.sku}}</div>
            </el-form-item>
            <el-form-item v-if="userInfo.source == 2" class="mg-t-15" label="物流渠道：">
              <div style="display: flex;">
                <el-select style="width: 300px;" v-model="setLotPriceDialog.logType" placeholder="请选择物流渠道">
                  <el-option label="全部订单使用同一个物流渠道计算" value="1"></el-option>
                  <el-option label="按订单的国家选择物流渠道计算" value="2"></el-option>
                </el-select>

                <div v-if="setLotPriceDialog.logType == 1">

                  <el-input placeholder="请填写物流渠道名称" maxlength="50" v-model="setLotPriceDialog.shipTypeStr" style="width: 200px;margin-left: 10px;"></el-input>
                </div>
              </div>
              <div v-show="setLotPriceDialog.logType == 2" class="countrylist">
                ...
                <div class="d-flex mg-t-10 countrylist-item" v-for="(cnt) in setLotPriceDialog.cntArr" :key="cnt.countryCode">
                  ...
                  <div class="countrylist-input">
                    <el-input placeholder="不填即该国订单不计算" maxlength="50" v-model="cnt.shipTypeStr"></el-input>
                  </div>
                </div>
              </div>
            </el-form-item>
            <el-form-item class="mg-t-15" label="当前采购单目标利润：">
              <div>
                <el-input v-model="setLotPriceDialog.numbers" style="width: 200px;"></el-input>
                %
              </div>
        ...
        <template v-if="setLotPriceDialog.step == 3">
          <div style="margin: 20px 50px;">
            <div class="mg-b-20 mg-t-20">
              设置成功：<span class="tx-success">{{setLotPriceDialog.results.success && setLotPriceDialog.results.success.cnt>=0 ? setLotPriceDialog.results.success.cnt : '---'}}</span> 个
            ...
            <div v-show="!setLotPriceDialog.successDetail">
              <p class="mg-b-10" v-for="(item,index) in setLotPriceDialog.results.success.detail">
                {{item.sku}}
              </p>
            </div>
            <div class="mg-b-20">
              设置失败：<span class="tx-danger">{{setLotPriceDialog.results.fail && setLotPriceDialog.results.fail.cnt>=0 ? setLotPriceDialog.results.fail.cnt : '---'}}</span> 个
              <el-link v-if="setLotPriceDialog.results.fail.cnt > 0" class="mg-l-10" type="primary" @click="setLotPriceDialog.failDetail = !setLotPriceDialog.failDetail">{{setLotPriceDialog.failDetail ? '查看' : '折叠'}}失败详情</el-link>
            </div>
            <div v-show="!setLotPriceDialog.failDetail">
              <p class="mg-b-10" v-for="(item,index) in setLotPriceDialog.results.fail.detail">
                {{item.country}} -	{{item.sku}} - {{item.logisticsName}}： {{item.message}}
              </p>
            </div>
          </div>
        </template>
        ...
        <el-divider></el-divider>
        <div slot="footer" class="dialog-footer">
          <el-button v-if="setLotPriceDialog.step == 2" type="primary" size="small" @click="saveSetLotPrice">批量设置</el-button>
          <el-button size="small" @click="setLotPriceDialog.isShow = false">关闭</el-button>
        </div>
      </el-dialog>
      ...
      >
        ...
        >
          ...
          <div>
            <el-button
              size="small"
              @click="setOtherERPDialog.isShow = false"
            >取消</el-button>
          </div>
        </div>
      </el-dialog>
      <!-- 其他ERP添加弹框 end  -->
      <!-- :title="drawerTitle"-->
      <el-drawer :size="'40%'" :destroy-on-close="true" :visible.sync="drawer" custom-class="drawer-outline" :wrapperClosable="false">
        ...
        <div v-loading="uploadLoading">
              ...
              <div style="margin-top: -30px;">
                <el-form label-position="right" label-width="120px">
                  <el-form-item :label="$t('invoice.deliveredProducts')">
                    <span class="tx-success">{{drawerinfo.deliveredCnt}}</span> {{$t('invoice.Sync')}}
                    <span class="tx-success">{{drawerinfo.syncCnt}}</span>{{$t('invoice.pro')}}
                  </el-form-item>
                  <el-form-item :label="$t('invoice.amount')">
                    US$ <span class="tx-success">{{$numberToCurrency(drawerinfo.deliveredAmount)}}</span>
                  </el-form-item>
                  <el-form-item :label="$t('invoice.notQuantity')">
                    <span class="tx-warning">{{drawerinfo.undeliveredCnt}}</span>{{$t('invoice.pro')}}
                  </el-form-item>
                  <el-form-item :label="$t('invoice.rate')">
                    <span class="tx-success">{{drawerinfo.deliverRate}}%</span>
                  </el-form-item>
                  <el-form-item :label="$t('invoice.wontQuantity')">
                    <span class="tx-error">{{drawerinfo.notDeliveredCnt}}</span>{{$t('invoice.pro')}}
                  </el-form-item>
                  <el-form-item :label="$t('invoice.wontAmount')">
                    US$ <span class="tx-error">{{Number(drawerinfo.notDeliveredSku) ? $numberToCurrency(drawerinfo.notDeliveredSku) : '0.00'}}</span>
                  </el-form-item>
                  <el-form-item :label="$t('invoice.notAmount')">
                    US$ <span class="tx-warning">{{Number(drawerinfo.undeliveredAmount) ? $numberToCurrency(drawerinfo.undeliveredAmount) : '0.00' }}</span>
                  </el-form-item>
                </el-form>
              </div>
            </el-card>
          ...
          <template v-if="drawertype == 2">
            <el-card v-if="invoiceInfo.payStatus == 2" class="box-card" shadow="never">
              <el-steps direction="vertical" :active="1">
                <el-step title="STEP 1">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.skuSet')}}</p>
                    <img src="images/step1.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
                <el-step title="STEP 2">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.priceSet')}}</p>
                    <img src="images/step2.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
                <el-step title="STEP 3">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.paymentSet')}}</p>
                    <img src="images/step3.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
                <el-step title="STEP 4">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.invoiceSend')}}</p>
                    <img src="images/step4.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
              </el-steps>
            </el-card>
            <el-card v-if="invoiceInfo.payStatus == 1" class="box-card" shadow="never">
              <el-steps direction="vertical" :active="1">
                <el-step title="STEP 1">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.ordersDownload')}}</p>
                    <img src="images/step1-1.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
                <el-step title="STEP 2">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.logisticsinformation')}}</p>
                    <img src="images/step1-2.png" width="100%" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
                <el-step title="STEP 3">
                  <template slot="description">
                    <p class="mg-b-10">{{$t('invoice.customerInform')}}</p>
                    <img src="images/url2-1.png" class="mg-b-20" style="box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" />
                  </template>
                </el-step>
              </el-steps>
            </el-card>
          ...
          <template v-if="drawertype == 3">
            <el-tabs type="border-card" v-model="activeName" @tab-click="handleClickTab">
                ...
                <el-card class="box-card" shadow="never" style="margin-bottom: 30px;">
                  <div>
                    ...
                    <div class="border-bottom-1 mg-b-15 pd-b-15">
                      <h6 class="mg-b-15" style="border-left: 4px solid #3F4EAE;padding-left: 10px;">
                        {{$t('myinvoice.customField')}}
                        <el-tooltip class="item" effect="dark" :content="$t('invoice.quantity0')" placement="right">
                          <i class="fa fa-question-circle-o mg-l-5"></i>
                        </el-tooltip>
                        <a style="color: #5C6AC4;" class="mg-l-20" href="javascript:;" @click="customWrapState()">{{showCustom ? $t('invoice.fieldsHide') : $t('invoice.fieldsShow')}}</a>
                      </h6>
                      <div class="row custom-wrap" :class="{hide: !showCustom}">
                        <el-row :gutter="20" style="margin: 0;">
                          <el-col :span="20" v-for="title in modalCustomExport.selectedTitles" :key="title.orName" style="margin-top: 10px;">
                            <div class="grid-content" style="display: flex;align-items: center;">
                              <label style="width: 200px;white-space: nowrap;text-align: right;margin-right: 10px;color: #999;">[{{title.orName}}] {{$t('myinvoice.showAs')}}</label>
                              <div class="col-6">
                                <el-input :placeholder="title.orName" size="small" v-model="title.name">
                    ...
                    <el-divider></el-divider>
                    <div class="text-left">
                      <el-button type="primary" size="small" @click="saveCustomExportSetting">{{$t('myinvoice.saveSetting')}}</el-button>
                      <template v-if="invoiceInfo.payStatus != 1">
                        <el-tooltip class="item" effect="dark" placement="top">
                          <div slot="content">
                      ...
                      <template v-else>
                        <el-button type="success" size="small" @click="saveCustomExport('1')">{{$t('invoice.exportExcel')}}</el-button>
                        <el-button type="success" size="small" @click="saveCustomExport('2')">{{$t('invoice.exportcsv')}}</el-button>
                      </template>
                    </div>
                  </div>
                </el-card>
              </el-tab-pane>
              <el-tab-pane :label="$t('invoice.shippinginformation')" name="second">
                ...
                <el-card class="box-card" shadow="never">
                  ...
                  <div style="margin-top: 40px;">
                    <el-upload class="upload-demo" ref="upload" action="" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" :auto-upload="false" :file-list="uploadfileList" :on-change="handleUploadChange">
              ...
              <el-tab-pane v-if="invoiceInfo.status != 3 && invoiceInfo.status != 4 && invoiceInfo.status != 5 && invoiceInfo.status != 6 && invoiceInfo.status != 9" :label="$t('invoice.ExportShopify')" name="third">
                <el-card class="box-card" shadow="never" style="margin-bottom: 30px;">

                  <div>
                    <el-button style="margin-left: 10px;" size="small" type="success" @click="saveCustomExportShopify">{{$t('invoice.ExportShopify')}}</el-button>

                  </div>
                </el-card>
              </el-tab-pane>
              <el-tab-pane v-if="invoiceInfo.status != 3 && invoiceInfo.status != 4 && invoiceInfo.status != 5 && invoiceInfo.status != 6 && invoiceInfo.status != 9" :label="$t('invoice.supplyinformation')" name="fourth">
                <!--style="box-shadow: 0 -2px 12px 0 rgba(0,0,0,.1);"-->
                <el-card class="box-card" shadow="never">
                  <div class="draw-alert mg-b-20">
                    <el-alert type="warning" :closable="false">
                  ...
                  <div style="margin-top: 40px;">
                    <template>
                      ...
                      <el-button style="margin-left: 10px;" size="small" type="success" @click="submitUpload">{{$t('invoice.supplyinformation')}}</el-button>
          ...
          <div class=" d-flex justify-content-center mg-t-20" style="padding: 30px;margin-bottom: 100px;">
            <el-divider></el-divider>
            <el-button type="primary" @click="drawer = false">{{drawertype == 3 ? $t('invoice.cancel') : $t('importdata.gotIt')}}</el-button>
          </div>
        </div>

      </el-drawer>
    </div>
    <v-tour name="invoiceTour" :steps="steps" :callbacks="myCallbacks" :options="myOptions">
      <template slot-scope="tour">
        <transition name="fade">
          <template v-for="(step, index) of tour.steps">
            <v-step v-if="tour.currentStep === index" :key="index+'tourStep'" :step="step" :previous-step="tour.previousStep" :next-step="tour.nextStep" :stop="tour.stop" :is-first="tour.isFirst" :is-last="tour.isLast" :labels="tour.labels" :highlight="tour.highlight">
              <template>
                <div slot="actions" class=" d-flex justify-content-between align-item-center pd-x-15">
                  <div v-if="tour.currentStep != steps.length - 1" class="close-tour">
                    <a style="color:#fff;" href="javascript:;" @click="stopDistributorTour">
                      <i class="fa fa-times" aria-hidden="true"></i>
                    </a>
                  </div>
                  <div class="btn tx-12 mg-r-30">
                    {{ $t('invoice.Step',{index: index + 1, length: steps.length}) }}
                  </div>
                  <div>
                    <el-button
                      v-if="index"
                      ...
                      size="mini" :id="`supplier-prev-${step.name}-${index + 1}`"
                      type="text" style="color:#fff;font-size: 12px;"
            ...
            <v-step v-if="tour.currentStep === index" :key="index+'tourStep'" :step="step" :previous-step="tour.previousStep" :next-step="tour.nextStep" :stop="tour.stop" :is-first="tour.isFirst" :is-last="tour.isLast" :labels="tour.labels" :highlight="tour.highlight">
              <template>
                <div slot="actions" class="pd-x-15">
                  <div v-if="tour.currentStep != steps.length - 1" class="close-tour">
                    <a style="color:#fff;" href="javascript:;" @click="stopConfirmTour">
                      <i class="fa fa-times" aria-hidden="true"></i>
                    </a>
                  </div>
                  <div class="tx-right">
                    <el-button @click="stopConfirmTour" id="stop-supplier-step" type="primary" size="mini" style="font-size: 12px;">{{$t('importdata.gotIt')}}</el-button>
                  </div>
                </div>
              </template>
            </v-step>
          ...
          <template v-for="(step, index) of tour.steps">
            <v-step v-if="tour.currentStep === index" :key="index+'tourStep'" :step="step" :previous-step="tour.previousStep" :next-step="tour.nextStep" :stop="tour.stop" :is-first="tour.isFirst" :is-last="tour.isLast" :labels="tour.labels" :highlight="tour.highlight">
              <template>
                <div slot="actions" class="pd-x-15">
                  <div v-if="tour.currentStep != steps.length - 1" class="close-tour">
                    <a style="color:#fff;" href="javascript:;" @click="stopConfirmTour1">
                      <i class="fa fa-times" aria-hidden="true"></i>
                    </a>
                  </div>
                  <div class="tx-right">
                    <el-button @click="stopConfirmTour1" id="stop-supplier-step" type="primary" size="mini" style="font-size: 12px;">{{$t('importdata.gotIt')}}</el-button>
                  </div>
                </div>
              </template>
            </v-step>
...
export default {
  ...
  data() {
    return {
      ...
      confirmsteps: [{
        ...
        content: `<div style='text-align:left;'>${this.$t('invoice.importerp')}</div>`,
        params: {
          placement: 'top',
          highlight: true
        }
      ...
      activities: [{
        ...
        content: '支持自定义Business address and contact details, title, and logo',
        timestamp: '',
        color: '#5c6ac4'
      }, {
        content: '默认样式的节点',
  ...
  watch: {
    ...
    realAmount() {
      if(Number(this.realAmount) < Number(this.totalAmount) && Number(this.realAmount) > 0) {
        this.discount = this.toFixed(Number(this.totalAmount) - Number(this.realAmount));
      } else {
        this.discount = 0;
      }
    },
  ...
  mounted() {
    window.addEventListener('message', (e)=>{
      ...
      let userId = localStorage.getItem("apiUserId") ? localStorage.getItem("apiUserId") : "",
        userToken = localStorage.getItem("apiUserToken") ? localStorage.getItem("apiUserToken") : "";
      if (!userId || !userToken) {
    ...
    this.shopData = localStorage.getItem("userInfo") ? JSON.parse(localStorage.getItem("userInfo")).erpShops : [];
    this.modalCustomExportDefault = JSON.stringify(this.modalCustomExport);

    $('body').on('click',   (event) =>  {
      var  target  =  $(event.target);
      if (!target.hasClass('el-popover') &&  target.parent('.el-popover').length  ===  0)  {
        $(".el-popover").remove();
      }
    });
  ...
  methods: {
    saveSetLotPrice(){
      if(this.userInfo.source == 2 && this.setLotPriceDialog.logType == 1 && !this.setLotPriceDialog.shipTypeStr){
      ...
      if(this.setLotPriceDialog.onlyNone == 2){
      ...
      this.setLotPriceCallBackTime = setInterval(() => {
        ...
        this.$apiCall("api.Invoice.checkPriceResult", {tempCode: this.tempCode}, r => {
          if(r.ErrorCode == "9999") {
...
//				this.$showLoading();
      if (step === 1) {
        ...
        let locationMall =
          window.location.host.toLowerCase().indexOf("sandbox") >= 0 ?
            ...
            "vendor.ourmall.com";
        let domainHost =
          http + "://" + locationMall + "/mabang-account.html";
        ...
        window.addEventListener("storage", e => {
          if(e.key == "authorizMabangAccount") {
            let mabangData = JSON.parse(localStorage.getItem("authorizMabangAccount"));
            this.$apiCall("api.User.mbLogin", {
              ...
              if(r.ErrorCode == "9999") {
                ...
                if(r.Data.Results.source == 3){//马帮3.0
                  this.getErpRules();
                }
                setTimeout(()=>{
                  this.$hideLoading();
        ...
        if(r.ErrorCode == "9999") {
          ...
          if(this.setLotPriceDialog.erpRules.length){
            this.setLotPriceDialog.erpRules.forEach((item)=>{
              this.$set(item,"shippingFeeRuleId",Number(item.shippingFeeRuleId));
            })
          }
        }
      })
    ...
    setLotPrice(item) {//打开设价模态框
      this.$showLoading();
      if(item && item.sku) {//sku设价
...
//			        	this.getErpRules();
//			        }
        this.$apiCall("api.Invoice.getPriceConfig", {
          invoiceId: this.invoiceInfo.id,
        }, r => {
          this.setLotPriceDialog.loading = false;
          if(r.ErrorCode == "9999") {
            this.setLotPriceDialog.isShow = true;
            this.setLotPriceDialog.shippingTypeArr = r.Data.Results.shippingTypeArr;
            ...
            this.setLotPriceDialog.ruleId = r.Data.Results.ruleId ? Number(r.Data.Results.ruleId) : "";
            this.setLotPriceDialog.price = r.Data.Results.priceType ? r.Data.Results.priceType : "1";
            if(this.setLotPriceDialog.price == 2){
              this.setLotPriceDialog.onlyNone = "";
            }
            ...
            if(r.Data.Results.shipTypeArr && r.Data.Results.shipTypeArr.length){
              let arr = [];
...
//								this.setLotPriceDialog.cntArr = r.Data.Results.shipTypeArr;
    ...
    getCntCountry() {
      this.loadCnt = true;
      this.$apiCall("api.Invoice.getItemsCountry", {
        invoiceId: this.invoiceInfo.id
      }, r => {
        this.loadCnt = false;
        if(r.ErrorCode == "9999") {
...
//								num: Number(r.Data.Results[k]),
    ...
    saveMarkShipped() {
      ...
      if(this.dialogMarkShipped.trackKey == 99 && !this.dialogMarkShipped.trackCompany) {
        this.$elementMessage(this.$t('invoice.logisticscompany'), "error");
        return;
      }
      ...
      this.$apiCall('api.Invoice.changeDeliver', {
        ...
        if(r.ErrorCode == "9999") {
          ...
          // })
          this.$apiCall("api.Invoice.getItems", {
            uniCode: this.invoiceInfo.uniCode,
            invoiceId: this.invoiceInfo.id,
            id: this.dialogMarkShipped.item.id,
          }, r => {
    ...
    handleClickTab(tab, event) {
      ...
      if(this.exportInvoiceType == "first" || this.exportInvoiceType == "third") {
        this.$showLoading();
        this.getExportTitle();
      }
    },
    ...
    isChangeSku(item) {
      if(this.invoiceInfo.timeApiGet || this.invoiceInfo.timeCreateOrder || item.fulfillStatus == 2 || this.invoiceInfo.status == 9) {
        return true;
      } else {
        return false;
      }
    ...
    isChangePrice(item) {
      if(this.invoiceInfo.timeApiGet || this.invoiceInfo.timeCreateOrder || item.fulfillStatus == 2 || this.invoiceInfo.status == 9 || this.invoiceInfo.customerRead == 1 || this.invoiceInfo.payStatus == 1) {
        return true;
      } else {
        return false;
      }
    ...
    isChangeOther(item) {
      if((item.isAbnormal == 2 && (this.invoiceInfo.timeApiGet || this.invoiceInfo.timeCreateOrder || item.fulfillStatus == 2 || this.invoiceInfo.status == 9 || this.invoiceInfo.customerRead == 1 || this.invoiceInfo.payStatus == 1))  || (item.isAbnormal == 1 && item.trackNumber)) {
        return true;
      } else {
        return false;
      }
    ...
    isChangeAmount() {
      if(this.invoiceInfo.timeApiGet || this.invoiceInfo.timeCreateOrder || this.invoiceInfo.status == 9 || this.invoiceInfo.customerRead == 1 || this.invoiceInfo.payStatus == 1) {
        return true;
      } else {
        return false;
      }
    ...
    downloadInvoice() {
      ...
      this.$apiCall("api.Invoice.getOne", {
        uniCode: this.uniCode
      }, r => {
        if(r.ErrorCode == "9999") {
          this.invoiceInfo = r.Data.Results;
          this.shopName = this.invoiceInfo.shopName ? this.invoiceInfo.shopName : "";
          this.invoiceData3 = [];
          this.getDownload(page, totalpage);
          //分享标题
          this.shareTitle = this.invoiceInfo.remark ? this.invoiceInfo.remark.title : "";
          this.shareDesc = this.invoiceInfo.remark ? this.invoiceInfo.remark.desc : "";
          ...
          this.platformRate = this.invoiceInfo.platformRate && this.invoiceInfo.platformRate > 0 ? this.invoiceInfo.platformRate : '';
          this.paymentType = this.invoiceInfo.paymentType ? this.invoiceInfo.paymentType : '';

          this.shippingAmount = this.invoiceInfo.shippingAmount ? this.toFixed(this.invoiceInfo.shippingAmount) : '';
          this.itemAmount = this.invoiceInfo.itemAmount ? this.toFixed(this.invoiceInfo.itemAmount) : '';
          this.totalAmount = this.invoiceInfo.totalAmount ? this.toFixed(this.invoiceInfo.totalAmount) : '';
          this.realAmount = this.invoiceInfo.realAmount ? this.toFixed(this.invoiceInfo.realAmount) : '';
          ...
          r.Data.Results.forEach((e, t) => {
            if(!e.price) {
              ...
              if(e.fulfillStatus != 2) {
                this.$set(e, "amount", (Number(e.quantity) * Number(e.price)).toFixed(2));
              } else {
    ...
    getCanvas() {
      ...
      html2canvas(copyDom[0], {
        ...
        var triggerDownload = $("<a>").attr("href", imgData).attr("download", "Invoice # " + this.invoiceInfo.code + ".jpg").appendTo("body");
    ...
    saveCustomExportSetting(type) {
      ...
      let apiUrl = this.exportInvoiceType == "third" ? "api.Invoice.saveInvoiceTitle" : "api.Invoice.saveExportTitle";
      this.$apiCall(apiUrl, {
        titles: obj
      }, r => {
        if(r.ErrorCode == "9999") {
          if(type > 0) {
            ...
            let apiUrl = this.exportInvoiceType == "third" ? "api.Invoice.exportInvoice" : "api.Invoice.exportOrder";
            this.$apiCall(apiUrl, {
              titles: obj,
              invoiceId: this.invoiceInfo.id,
    ...
    downloadFile() {
      let url = this.exportInvoiceType == "fourth" ? "./file/shopify_orders_import.csv" : "./file/orders_impor.xlsx";
    ...
    querySearch(queryString, cb) {
      ...
      var results = queryString ? shopData.filter(this.createFilter(queryString)) : shopData;
    ...
    getExportTitle() {
      ...
      let apiUrl = this.exportInvoiceType == "third" ? "api.Invoice.exportInvoiceTitle" : "api.Invoice.exportTitle";
    ...
    saveCustomExportSettingOnly(type) {
      ...
      this.$apiCall("api.Invoice.saveExportTitle", {
        titles: obj
      }, r => {
        this.$hideLoading();
        if(r.ErrorCode == "9999") {
    ...
    confirmMoneyFn() {
      this.$showLoading();
      this.$apiCall("api.Invoice.setSyncToErp", {
        invoiceId: this.invoiceInfo.id,
    ...
    confirmMoneyOnly(type) {
      this.$showLoading();
      this.$apiCall("api.Invoice.setPayed", {
        invoiceId: this.invoiceInfo.id,
        shopName: this.shopName
      }, r => {
        this.$hideLoading();
    ...
    sendInvoice() {
      this.loading = true;
      this.$apiCall('api.Invoice.checkItemPrice', {invoiceId: this.invoiceInfo.id}, r => {
        this.loading = false;
        if (r.ErrorCode == "9999") {
          let re = r.Data.Results;
    ...
    setPayment() {
      ...
      //				}
      this.$showLoading();
      this.$apiCall("api.Invoice.setPayment", {
        invoiceId: this.invoiceInfo.id,
    ...
    getSkuInfo2(item, index, type) {
      ...
      window.lazyGetSkuInfo2 = setTimeout(() => {
        this.$apiCall("api.Invoice.getSameCnt", {
          invoiceId: this.invoiceInfo.id,
    ...
    changeShippingAmount() {
      this.$apiCall("api.Invoice.changeAmount", {
        invoiceId: this.invoiceInfo.id,
        shippingAmount: this.shippingAmount,
      }, r => {
        if(r.ErrorCode == "9999") {
          this.shippingAmount = r.Data.Results.shippingAmount ? this.toFixed(r.Data.Results.shippingAmount) : '';
          this.totalAmount = r.Data.Results.totalAmount ? this.toFixed(r.Data.Results.totalAmount) : '';
          this.realAmount = r.Data.Results.realAmount ? this.toFixed(r.Data.Results.realAmount) : '';
          if(Number(this.realAmount) < Number(this.totalAmount) && Number(this.realAmount) > 0) {
            this.discount = this.toFixed(Number(this.totalAmount) - Number(this.realAmount));
          } else {
            this.discount = 0;
    ...
    setStop(item, status) {
      ...
      this.$apiCall("api.Invoice.setFulfill", params, r => {
        this.$hideLoading();
        if(r.ErrorCode == "9999") {
          this.itemAmount = r.Data.Results.itemAmount ? this.toFixed(r.Data.Results.itemAmount) : '';
          this.totalAmount = r.Data.Results.totalAmount ? this.toFixed(r.Data.Results.totalAmount) : '';
          this.realAmount = r.Data.Results.realAmount ? this.toFixed(r.Data.Results.realAmount) : '';
          $(".el-popover").remove();
          this.$elementMessage(this.$t('invoice.successModification'), 'success');
      ...
      window.lazySkuBatch = setTimeout(() => {
        ...
        this.$apiCall("api.Invoice.setSku", params, r => {
          if(r.ErrorCode == "9999") {
            ...
            if(this.form.resource == 1) { //未设置SKU的同款
              this.invoiceData.forEach((e, t) => {
                if(e.customerSku == item.customerSku && !e.sku && e.fulfillStatus != 2) {
    ...
    changeSkuFn(item, index, type) { //更换sku
      if(!this.invoiceTour) return;
      if(!item.sku) {
        this.$elementMessage(this.$t('invoice.fillsku'), 'error');
        return;
      ...
      window.lazyPriceBatch = setTimeout(() => {
        ...
        this.$apiCall("api.Invoice.setPrice", params, r => {
          if(r.ErrorCode == "9999") {
            ...
            if(this.formPrice.resource == 1) { //未设置SKU的同款
              this.invoiceData.forEach((e, t) => {
                if(e.customerSku == item.customerSku && !e.price && e.fulfillStatus != 2) {
    ...
    changePriceFn(item, i, type) { //修改价格
      if(!this.invoiceTour) return;
      if(!item.price) return;
      if(window.lazyPrice) {
    ...
    changePriceTotalFn(item, i) { //修改价格
      if(!this.invoiceTour) return;
    ...
    changePriceTotalFnBySub(props, item) { //修改价格
        ...
        if(s.fulfillStatus != 2) {
          if(type == "sku") {
            ...
            if(!s.price) {
              ...
              if(s.fulfillStatus != 2) {
                this.$set(s, "amount", (Number(s.quantity) * Number(s.price)).toFixed(2));
              } else {
    ...
    removelogo() {
      ...
      this.$apiCall("api.User.changeByUser", {
        invoiceId: this.invoiceInfo.id,
        logo: "",
      }, r => {
    ...
    tabcChange(i) {
      ...
      if(i == 5) {
        this.getOperateLog(this.invoiceInfo.id); //获取时间轴
        this.getMessages(this.invoiceInfo.id); //获取消息
        //					$('.el-scrollbar__wrap').animate({scrollTop: $('#messageContainer').offset().top}, 1000);
        return;
      }
      switch(i) {
        case 1:
    ...
    getCnt(id) {
      this.$apiCall("api.Invoice.getItemsCnt", {
        invoiceId: id,
      }, r => {
        if(r.ErrorCode == "9999") {
      ...
      this.$apiCall("api.Invoice.getOperateLog", {
        ...
        if(r.ErrorCode == "9999") {
          ...
          this.operateLog.forEach((e) => {
            ...
            this.$set(e, "datetime", this.$moment(e.time * 1000).format("h:mm a"));
          })
      ...
      this.$apiCall("api.Invoice.getMessages", {
        ...
        if(r.ErrorCode == "9999") {
          ...
          this.messages.forEach((e) => {
            ...
            this.$set(e, "vname", !e.vendorName ? "n" : e.vendorName.substr(0, 1));
            this.$set(e, "cname", !e.customerName ? "n" : e.customerName.substr(0, 1));
          })
        ...
        if(!arr) {
          ...
          for(let i = 0; i < arr.length; i++) {
            ...
            if(names && names[1]) {
              ...
              msgContent = msgContent.replace(arr[i], 'emoji' + names[1] + 'emoji')
            }
          }
        };
      }
      this.$showLoading();
      this.$apiCall("api.Invoice.addMessage", {
        invoiceId: this.invoiceInfo.id,
        content: msgContent,
        images: arr
      }, r => {
    ...
    getDataAll() { //初次加载
      ...
      this.$apiCall("api.Invoice.getOne", {
        ...
        if(r.ErrorCode == "9999") {
          this.invoiceInfo = r.Data.Results;
          this.getCntCountry();
          this.showItemPrice = this.invoiceInfo.showItemPrice == '1' ? true : false;
          if(this.invoiceInfo.code == "20010699") {
            ...
            if(!userId || !userToken) {
              if(this.iframeId !== this.invoiceInfo.customerId) {
                ...
                this.loginShow = true;
                this.loginRId = this.invoiceInfo;
                if(Number(this.invoiceInfo.vendorId) > 0) { //登录
                  this.loginType = 1;
                } else { //注册
                  this.loginType = 2;
                }
    ...
    handleChange(item, value) {
      setTimeout(() => {
        this.$apiCall("api.Invoice.changeQuantity", {
          invoiceId: this.invoiceInfo.id,
          itemId: item.id,
    ...
    changePriceFnBySub(props, item) { //修改价格
      if(!item.price) return;
      let index = props.$index;
    ...
    submitAddcustomer(formName) {
      this.$refs[formName].validate((valid) => {
        if(valid) {
          ...
          this.$apiCall("api.Relationship.addByVendor", {
    ...
    editCompany() {
      ...
      this.formEditCompany.name = this.invoiceInfo.vendorName ? this.invoiceInfo.vendorName : "";
      this.formEditCompany.address = this.invoiceInfo.address ? this.invoiceInfo.address : "";
    },
    ...
    editCustomer() {
      ...
      this.formEditcustomer.name = this.invoiceInfo.customerName ? this.invoiceInfo.customerName : "";
      this.formEditcustomer.email = this.invoiceInfo.customerEmail ? this.invoiceInfo.customerEmail : "";
      this.formEditcustomer.remark = this.invoiceInfo.customerRemark ? this.invoiceInfo.customerRemark : "";
    ...
    submitEditcustomer(formName) {
      this.$refs[formName].validate((valid) => {
        if(valid) {
          this.$showLoading(".editCustomer");
          this.$apiCall("api.Relationship.changeByVendor", {
            ...
            this.$hideLoading();
            if(r.ErrorCode == "9999") {
              this.$elementMessage(this.$t('invoice.successCustomer'), 'success');
    ...
    selectCustomer(item) {
      this.$showLoading();
      this.$apiCall("api.Invoice.changeCustomer", {
        invoiceId: this.invoiceInfo.id,
    ...
    changeCustomerName() {
      ...
      window.lazy = setTimeout(() => {
        this.$apiCall("api.Relationship.getByName", {
    ...
    tableRowClassName({row, rowIndex}) {
      if(this.invoiceData[rowIndex] && this.invoiceData[rowIndex].fulfillStatus == 2) {
        return 'warning-row'
      } else {
        return ''
      }
    ...
    tableRowClassName2({row, rowIndex}) {
      if(this.invoiceData[rowIndex] && this.invoiceData[rowIndex].fulfillStatus == 2) {
        if(this.invoiceData[rowIndex].allItems && this.invoiceData[rowIndex].allItems.length > 0)
          return 'showrow warning-row'
        else
          return 'hiderow warning-row'
      } else {
        if(this.invoiceData[rowIndex].allItems && this.invoiceData[rowIndex].allItems.length > 0)
          return 'showrow'
        else
      ...
      this.$apiCall(
        ...
        (r) => {
          ...
          if (r.ErrorCode == 9999) {
            ...
            this.setLotPriceDialog.isShow = false
            this.setOtherERPDialog.contents = ""
            this.$message({ message: "我们已收到您的留言！我们将尽快在“我的OurMall”>>“客服支持”页面回复您。", type: "success" });
          } else {
...
.set-price .logo-wrap{
  ...
  height: 130px;
  border-radius: 100%;
  background: rgb(189,32,41) url("/images/main_logo.png") no-repeat center center;
  background-size: 80%;
  margin: 20px auto;
}
