<template>
    <div class="list-view">
        <h2 class="list-title">{{data.listTitle || 'Collection List'}}</h2>
        <div class="more-but">{{data.moreText || 'More'}} <i class="el-icon-arrow-right"></i></div>
        ...
        <div 
            ...
            <div class="inner" v-if="index<(data.rowNum*data.colNum)">
                ...
                <div v-if="data.displayPrice" class="product-cost">$ {{item.cost}}</div>
                </div>
            </div>
        </div>
        </div>
