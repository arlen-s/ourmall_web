  ...
  <div class="chat-top-wrap" :class="{max: $route.name == 'chat'}" style="height: 100%;">
    <div class="title">
      <div class="left">
        ...
        <div class="welcome">Hi, welcome to OurMall</div></div>
      <div class="right">
		 
       <a
         v-if="($route.name != 'chat' || isIframe) && $store.state.userInfo"
         href="javascript:;"
         @click="openNewWinChat"
       >
	   <el-tooltip class="item" effect="dark" content="Open in new tab" placement="bottom-end">
    ...
    <div class="content-wrap" v-if="isShow">
      ...
      <template v-else>
        ...
        <p>
          Welcome to OurMall! Please sign in or register an account to
          start the chat.
        </p>
        <div class="login-action">
          <button class="btn signin" @click="signReg(1)">
            Sign in
          </button>
          <button class="btn reg" @click="signReg(2)" v-if="$root.$children[0].baseUrl != '/admin'">
            Register
          </button>
        </div>
      </template>
      ...
      <div>
        ...
        <div class="submit-wrap">
          <div class="send" type="button" @click="sendMsg">
            <img src="../../public/images/v2/send.png">
          </div>
        </div>
      </div>
    </div>
    ...
    >  
    <!-- 预览 -->
    <div v-if="viewImg.length" class="app-images-arr" v-viewer="{}">
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      this.sound = localStorage.getItem('chatSound') == 0 ? Number(localStorage.getItem('chatSound')) : 1;
      this.isShow = true
    });
    this.getItems();
    localStorage.setItem('firstGetNewCnt', true)
      if(this.isShow){
    ...
    let changeStorage = (e)=>{
      if(e.key == 'chatSound'){
        this.sound = Number(localStorage.getItem('chatSound'))
      }
    }
  ...
  methods: {
    signReg(s){
      if(s==1){
         this.$root.$children[0].$refs['mainRouter'].toSignin();
      }else{
         this.$root.$children[0].$refs['mainRouter'].isReg = '2';
      }
      this.closeChatWin();
    },
    ...
    chageSound() {
      ...
      localStorage.setItem('chatSound', this.sound);
    },
    closeChatWin() {
      //最小化聊天窗口
      if(this.isIframe){
...
.content-wrap {
  ...
  .login-action {
    ...
    .signin:hover {
	  ...
	  border: 2px solid $primaryHoverColor!important;
    }
    .reg {
      background-color: #fff;
	  border: 2px solid $primaryHoverColor!important;
