          ...
          <template slot-scope="scope">
            ...
            <template v-if="scope.row.status == '1'">
              <br>
                ...
                <el-link type="primary" v-if="vendorId != 148982 && vendorId != 146428&& vendorId != 144875&& vendorId != 144843&& vendorId != 143779&& vendorId != 143654&& vendorId != 74" style="font-size: 12px;" :href="`//${host.indexOf('sandbox') > -1 ? 'sandboxshop' : 'shop'}${scope.row.shopId}.bcndropshippingelite.com/item/${scope.row.id}/${scope.row.name}.html`" target="_blank">
                {{$t('goods.预览')}}
              </el-link>
              <el-link type="primary" v-else style="font-size: 12px;" :href="`//${host.indexOf('sandbox') > -1 ? 'sandboxshop' : 'shop'}${scope.row.shopId}.bcndropshippingelite.com/itemOld/${scope.row.id}/${scope.row.name}.html`" target="_blank">
                {{$t('goods.预览')}}
              </el-link>
