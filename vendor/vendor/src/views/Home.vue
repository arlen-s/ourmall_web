        ...
        <div v-else class="box-content">
          ...
          <template v-if="isReg == '1'">
            ...
            <template>
              ...
              <div class="btn-row">
                <el-button
                  :class="{ active: tabAction == 'login' }"
                  @click="handleTab('login')"
                  >密码登录</el-button
                >
                <el-button
                  :class="{ active: tabAction == 'codeLogin' }"
                  @click="handleTab('codeLogin')"
                  >验证码登录</el-button
                >
              </div>
              <!-- 密码登录 -->
              ...
              <div v-if="tabAction == 'codeLogin'" class="login-box">
                ...
                >
                    ...
                    <vue-simple-verify
                      ...
                      :successTips="`${
                        regStatus.loading
                          ? 'Loading...'
                          : $t('signin.重新获取验证码', {
                              second: regStatus.mobileSeconds,
                            })
                      }`"
                      movedColor="#5c6ac4"
                    /> 
                  ...
                  <div v-if="regStatus.success" class="success">
                    {{ regStatus.success }}
                  </div>
                  <div v-if="loginStatus.error" class="error">
                    {{ loginStatus.error }}
              ...
              <div class="action-row">
                ...
                <div class="a-row2">
                  ...
                  >
                  <span
                    v-if="$root.$children[0].baseUrl != '/admin'"
                    style="margin: 0 10px"
                    >|</span
                  >
                  <el-link
              ...
              >
                ...
                <el-form-item prop="password" style="max-width: 360px">
                  <el-input
                    ...
                    :disabled="regStatus.loading"
                    show-password
                    :placeholder="$t('signin.* 请输入密码')"
                  >
                  </el-input>
                  <p style="color: #909399; font-size: 12px; line-height: 20px">
                    <i class="el-icon-info"></i
                    >{{ $t("signin.（6-20位，需包涵英文字母和数字）") }}
                  </p>
                </el-form-item>
                ...
                >
                  <el-form-item style="margin-bottom: 0">
                    <el-button
                      ...
                      style="width: 360px; height: 68px; font-size: 18px"
                      @click="registers"
                ...
                <div class="tx-center mg-b-20" style="max-width: 360px">
                  <el-link
                    class="d-inline"
                    @click="toSignin()"
                    style="font-size: 12px; color: #8292fa"
                  >
      ...
      rules: {
        ...
        password: [
          ...
          },
          {
            ...
            message: this.$t(
              "signin.密码必须包含字母、数字和特殊字符，至少6个字符"
            ),
            trigger: "blur",
          },
        ],
  ...
  methods: {
    ...
    handleLogin(Results, type) {
      ...
      localStorage.setItem(
        "apiSubUserId",
        Results.subUserId ? Results.subUserId : ""
      );
      ...
      if (localStorage.getItem("wantVisitPathSu")) {
        ...
        if (reg.test(window.location.host)) {
          ...
          if (
            ...
            (!e.subItem ||
              e.subItem.some((obj) => {
                return obj.role && this.$isRole(obj.role);
              }))
          ) {
            let url;
            ...
            let e = this.$root.$children[0].navMenu.find((item) => {
              return (
                item.role &&
                this.$isRole(item.role) &&
                (!item.subItem ||
                  item.subItem.some((obj) => {
                    return obj.role && this.$isRole(obj.role);
                  }))
              );
            });
